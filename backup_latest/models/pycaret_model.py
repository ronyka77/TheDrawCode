...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    feature_matrix, feature_defs = ft.dfs(
        ...
        max_depth=3,
        verbose=True,
        ignore_columns={'fixture_data': ['is_draw']},
        drop_contains=['%>', '&'],
...
def train_with_pycaret(feature_matrix: pd.DataFrame):
    ...
    best_model = clf.compare_models(sort="Precision")

    # Tune the best model for precision
    tuned_model = clf.tune_model(best_model, optimize="Precision")

    # Evaluate the tuned model
    clf.evaluate_model(tuned_model)

    # Save the final model
    clf.save_model(tuned_model, "soccer_draw_model_pycaret")
