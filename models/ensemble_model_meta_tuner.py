...
class EnsembleModel(BaseEstimator, ClassifierMixin):
    ...
    def _compute_dynamic_weights(self, targets: pd.Series, p_xgb: np.ndarray, p_cat: np.ndarray, p_lgb: np.ndarray) -> dict:
        ...
        self.logger.info(f"XGBoost validation precision: {prec_xgb:.4f}")
        self.logger.info(f"CatBoost validation precision: {prec_cat:.4f}")
        self.logger.info(f"LightGBM validation precision: {prec_lgb:.4f}")
        ...
        self.logger.info(f"XGBoost dynamic weight: {weights['xgb']:.4f}")
        self.logger.info(f"CatBoost dynamic weight: {weights['cat']:.4f}")
        self.logger.info(f"LightGBM dynamic weight: {weights['lgb']:.4f}")
    ...
    def evaluate(self, X: pd.DataFrame, y: pd.Series) -> dict:
        ...
        try:
            ...
            auc = None
            self.logger.info(f"AUC computation failed: {str(e)}")
        ...
        self.logger.info(f"Required recall threshold: {self.required_recall:.4f}")
        self.logger.info(f"Recall meets required threshold: {recall_flag}")
        
        if not recall_flag:
            self.logger.info(f"Recall {recall:.4f} is below required threshold {self.required_recall:.4f}")
        else:
            self.logger.info(f"Recall {recall:.4f} meets required threshold {self.required_recall:.4f}")
...
def train_base_models_once(
    ...
    try:
        ...
        logger.error(f"Error in train_base_models_once: {str(e)}")
        raise
...
def hypertune_meta_learner(logger: ExperimentLogger = None) -> Dict[str, Any]:
    ...
    try:
                ...
                for n_est in meta_params_grid["n_estimators"]:
                    for max_depth in meta_params_grid["max_depth"]:
                        for scale_pos_weight in meta_params_grid["scale_pos_weight"]:
                            ...
                            logger.info(f"Number of estimators: {n_est}")
                            ...
                            try:
                                ...
                                logger.info(f"Candidate {current_combination} evaluation results:")
                                ...
                                logger.info(f"XGBoost weight: {ensemble_model.dynamic_weights['xgb']:.4f}")
                                logger.info(f"CatBoost weight: {ensemble_model.dynamic_weights['cat']:.4f}")
                                logger.info(f"LightGBM weight: {ensemble_model.dynamic_weights['lgb']:.4f}")
        ...
        logger.error(f"Error in hypertune_meta_learner: {str(e)}")
        raise
...
if __name__ == "__main__":    
    ...
    if best_meta_config is not None:
        logger.info("Best meta learner configuration found:")
        for param_name, param_value in best_meta_config["meta_params"].items():
