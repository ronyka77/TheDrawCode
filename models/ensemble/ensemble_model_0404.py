...
from models.ensemble.meta_features_0404 import create_meta_features_optimized, create_meta_dataframe
...
class EnsembleModel(BaseEstimator, ClassifierMixin):
    def __init__(self, 
        ...
        self.logger = logger or ExperimentLogger(experiment_name="ensemble_model_0404", log_dir="./logs/ensemble_model_0404")
        ...
        self.xgb_run_id = 'b67bb1b7722b42ca945722693ebec496' # Keep existing
        ...
        self.extra_run_id = '0e61e8e5017540a2bc754b7895d4eb92' # Keep existing (now standard)
        ...
        if self.dynamic_weighting:
            ...
            self.dynamic_weights = {'xgb': 1/5, 'tabnet': 1/5, 'lgb': 1/5, 'extra': 1/5, 'mlp': 1/5}
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, X_test=None, y_test=None, split_validation=True, val_size=0.2) -> dict:
        ...
        self.logger.info("Obtaining test predictions from base models (for meta-learner training)...")
        ...
        if self.dynamic_weighting:
            ...
            self.dynamic_weights, self.thresholds = compute_precision_focused_weights_optimized(
                p_xgb_val, p_tabnet_val, p_lgb_val, p_extra_val, p_mlp_val, # Added MLP
                y_val, self.target_precision, self.min_recalls, self.logger
            )
            ...
            self.dynamic_weights_train, self.thresholds_train = compute_precision_focused_weights_optimized(
                p_xgb_test, p_tabnet_test, p_lgb_test, p_extra_test, p_mlp_test, # Added MLP
                y_test, self.target_precision, self.min_recalls, self.logger
            )
        ...
        self.meta_learner = hypertune_meta_learner(
            meta_df_train, y_test, # Train/evaluate HPO on test set 
            meta_df_val, y_val,    # Use validation set for final HPO eval (or nested CV split)
        ...
        self.logger.info(f"Tuning final threshold using validation data for target precision {self.target_precision}...")
    ...
    def predict_proba(self, X) -> np.ndarray:
        ...
        try:
            ...
            meta_features = create_meta_features_optimized(
                p_xgb, p_tabnet, p_lgb, p_extra, p_mlp, # Added MLP
                self.dynamic_weights if self.dynamic_weighting else None, 
                self.thresholds if self.dynamic_weighting else None
            )
    ...
    def load_models_from_mlflow(self, 
        ...
        try:
            ...
            if xgb_pyfunc.metadata.signature and xgb_pyfunc.metadata.signature.inputs:
                ...
                self.logger.info(f"Updated XGBoost feature signature: {len(self.xgb_features)} features")
            else:
        ...
        try:
            ...
            if lgb_pyfunc.metadata.signature and lgb_pyfunc.metadata.signature.inputs:
                ...
                self.logger.info(f"Updated LightGBM feature signature: {len(self.lgb_features)} features")
            else:
        ...
        try:
            ...
            self.model_tabnet = mlflow.sklearn.load_model(tabnet_uri) # Assuming saved via sklearn flavor
            ...
            if tabnet_pyfunc.metadata.signature and tabnet_pyfunc.metadata.signature.inputs:
                ...
                self.logger.info(f"Updated TabNet feature signature: {len(self.tabnet_features)} features")
            else:
        ...
        try:
            ...
            if extra_pyfunc.metadata.signature and extra_pyfunc.metadata.signature.inputs:
                ...
                self.logger.info(f"Updated Extra Trees signature: {len(self.extra_features)} features")
            else:
            ...
            raise ValueError(f"Failed to load Extra Trees model: {str(e)}")

        # Load MLP (sklearn) model
        if not self.mlp_run_id or self.mlp_run_id == 'PLACEHOLDER_MLP_RUN_ID':
            self.logger.warning("MLP Run ID is not set or is a placeholder. Skipping MLP model loading.")
        else:
            try:
                ...
                if mlp_pyfunc.metadata.signature and mlp_pyfunc.metadata.signature.inputs:
                    ...
                    self.logger.info(f"Updated MLP feature signature: {len(self.mlp_features)} features")
                else:
                ...
                self.logger.info(f"Loading MLP scaler artifact '{mlp_scaler_path}' from run {self.mlp_run_id}...")
                scaler_local_path = mlflow.artifacts.download_artifacts(run_id=self.mlp_run_id, artifact_path=mlp_scaler_path)
                ...
                raise ValueError(f"Failed to load MLP model or scaler: {str(e)}")
