...
def create_meta_features_optimized(p_xgb: np.ndarray, p_tabnet: np.ndarray, 
    ...
    if not all(len(p) == len(p_xgb) for p in [p_tabnet, p_lgb, p_extra]):
    ...
    if dynamic_weights:
        weighted_avg = (
            ...
            dynamic_weights.get('tabnet', 0.25) * p_tabnet +
            dynamic_weights.get('lgb', 0.25) * p_lgb +
            dynamic_weights.get('extra', 0.25) * p_extra
        )
    else:
        # Simple average
        weighted_avg = (p_xgb + p_tabnet + p_lgb + p_extra) / 4.0
    ...
    diff_extra_tabnet = np.abs(p_extra - p_tabnet)
    ...
    meta_features = np.column_stack([
        ...
        diff_extra_tabnet.reshape(-1, 1),
        diff_extra_lgb.reshape(-1, 1)
    ])
    
    # Add max and min probabilities
    max_prob = np.maximum.reduce([p_xgb, p_tabnet, p_lgb, p_extra])
    min_prob = np.minimum.reduce([p_xgb, p_tabnet, p_lgb, p_extra])
    ...
    for i in range(n_samples):
        probs = [p_xgb[i], p_tabnet[i], p_lgb[i], p_extra[i]]
...
def create_meta_features(p_xgb: np.ndarray, p_tabnet: np.ndarray, 
                        p_lgb: np.ndarray, p_extra: np.ndarray, 
    ...
    votes = np.column_stack([
        ...
        (p_tabnet > vote_threshold).astype(int),
...
def create_meta_dataframe(meta_features: np.ndarray) -> pd.DataFrame:
    ...
    """
    column_names = [
        'prob_xgb', 'prob_tabnet', 'prob_lgb', 'prob_extra',
        'weighted_avg',
        'diff_xgb_tabnet', 'diff_xgb_lgb', 'diff_tabnet_lgb',
        'diff_extra_xgb', 'diff_extra_tabnet', 'diff_extra_lgb',
        'max_prob', 'min_prob', 'range_prob',
        'rank_xgb', 'rank_tabnet', 'rank_lgb', 'rank_extra',
