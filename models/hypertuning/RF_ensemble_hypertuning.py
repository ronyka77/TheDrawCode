"""
Random Forest model implementation for binary classification with global training capabilities.
...
try:
    ...
    print(f"Project root RF_ensemble_hypertuning: {project_root}")
except Exception as e:
    print(f"Error setting project root path: {e}")
    # Fallback to current directory if path resolution fails
    sys.path.append(os.getcwd().parent.parent)
    print(f"Current directory RF_ensemble_hypertuning: {os.getcwd().parent.parent}")
os.environ['GIT_PYTHON_GIT_EXECUTABLE'] = "C:/Program Files/Git/bin/git.exe"

# Local imports
from utils.logger import ExperimentLogger
experiment_name = "RF_ensemble_hypertuning"
logger = ExperimentLogger(experiment_name=experiment_name, log_dir='./logs/RF_ensemble_hypertuning')
...
class GlobalHypertuner:
    """Global hyperparameter tuner for Random Forest model optimizing for validation metrics."""
    MIN_SAMPLES: int = 1000
    DEFAULT_THRESHOLD: float = 0.50
    TARGET_PECISION: float = 0.5
    ...
    def __init__(self, 
                ...
                target_precision: float = TARGET_PECISION,
                target_recall: float = TARGET_RECALL,
                precision_weight: float = PRECISION_WEIGHT,
                hyperparam_spec: Dict = HYPERPARAM_SPEC):

        self.logger = logger or ExperimentLogger(experiment_name="RF_ensemble_hypertuning", log_dir='./logs/RF_ensemble_hypertuning')
    ...
    def objective(self, 
        ...
        try:
            ...
            best_score = -np.inf
            patience = trial.suggest_int('patience', 2, 10)  # Number of iterations to wait for improvement
            no_improvement_count = 0
            min_estimators = 100  # Minimum number of estimators to train
            
            # Incremental training with early stopping
            for i in range(min_estimators, param['n_estimators'] + 1, 100):
                model.n_estimators = i
                model.fit(features_train, target_train)
                
                # Get validation score
                threshold, metrics = self._find_optimal_threshold(model, features_val, target_val)
                current_score = metrics['precision']
                # Focus solely on precision if recall > 20%, otherwise penalize
                ...
                if no_improvement_count >= patience:
                    self.logger.info(f"Early stopping at {i} estimators")
                    break
    ...
    def evaluate_model(self,
        ...
        try:
            ...
            # Use Random Forest specific parameters
...
def tune_global_model():
    ...
    try:
        ...
        # Start MLflow run
        with mlflow.start_run(run_name=f"ensemble_rf_tuning_{datetime.now().strftime('%Y%m%d_%H%M')}"):
            # Log data statistics
            mlflow.log_params({
                "target_precision": 0.50,
                "target_recall": 0.20,
            ...
            best_params = hypertuner.tune_model(
                X_train, y_train, X_val, y_val, X_test, y_test, n_trials=20
            )
