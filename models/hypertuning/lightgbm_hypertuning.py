...
def run_lightgbm_hypertuning() -> Dict[str, Any]:
    """Run hyperparameter tuning for LightGBM model using soccer prediction data.
    ...
    try:
        ...
        data_loader = DataLoader(experiment_name="lightgbm_hypertuning")
        ...
        try:
            ...
            try:
                ...
                if isinstance(best_params, tuple):
                    ...
                    model_config = model.config_loader.load_model_config('lightgbm')
            ...
            for key in ['seed', 'n_estimators', 'random_seed', 'early_stopping_rounds', 'num_leaves', 'max_depth', 'min_data_in_leaf', 'bagging_freq']:
            ...
            final_model = LightGBMModel(
                experiment_name="lightgbm_hypertuning_final",
                model_type="lightgbm",
                logger=exp_logger
            )
        ...
        results_file = results_path / f"lightgbm_hypertuning_{timestamp}.json"
        ...
        if results:
            ...
            results_path.mkdir(parents=True, exist_ok=True)
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            error_file = results_path / f"lightgbm_hypertuning_error_{timestamp}.json"
