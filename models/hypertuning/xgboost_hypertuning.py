"""Script to run hyperparameter tuning for XGBoost model."""
...
try:
    ...
    from models.StackedEnsemble.base.tree_based.xgboost_model import XGBoostModel
    from models.StackedEnsemble.shared.data_loader import DataLoader
    
except Exception as e:
    raise ImportError(f"Error importing modules: {str(e)}")

def run_xgboost_hypertuning() -> Dict[str, Any]:
    """Run hyperparameter tuning for XGBoost model using actual soccer prediction data.
    ...
    try:
        exp_logger.info("Starting XGBoost hyperparameter tuning with soccer prediction data")
        
        # Load actual soccer prediction data
        exp_logger.info("Loading soccer prediction data")
        data_loader = DataLoader(experiment_name="xgboost_hypertuning")
        ...
        model = XGBoostModel(experiment_name="xgboost_hypertuning", model_type="xgboost", logger=exp_logger)
        ...
        if results:
            ...
            results_path.mkdir(parents=True, exist_ok=True)
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            error_file = results_path / f"xgboost_hypertuning_error_{timestamp}.json"
