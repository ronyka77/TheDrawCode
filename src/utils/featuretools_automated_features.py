...
class SoccerFeaturetoolsEngineer:
    ...
    def __init__(
        ...
        self.logger.info("SoccerFeaturetoolsEngineer initialized with optimizations", 
                        extra={
                            'max_depth': max_depth,
    ...
    def _setup_primitives(self) -> None:
        ...
        self.soccer_feature_groups = {
            ...
            'form_features': [
                ...
                'form_stability', 'form_difference',
                'home_xg_form', 'away_xg_form'
            ],
            'tactical_features': [
                'Home_possession_mean', 'away_possession_mean',
                'home_corners_rollingaverage', 'away_corners_rollingaverage',
                'home_fouls_rollingaverage', 'away_fouls_rollingaverage'
            ],
            'elo_features': [
                'home_team_elo', 'away_team_elo',
                'elo_difference', 'elo_similarity'
            ]
        }
    ...
    def prepare_data_for_featuretools(self, df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            self.logger.info("Preparing data for featuretools entity creation", 
            ...
            categorical_candidates = ['home_encoded', 'away_encoded', 'venue_encoded', 'league_encoded']
            ...
            if original_fixture_count != total_rows:
                self.logger.warning(f"Non-unique fixture_ids detected: {original_fixture_count} unique vs {total_rows} total")
                self.logger.info("Creating unique fixture_id sequence for featuretools compatibility")
                ...
                self.logger.info(f"Generated unique fixture_id sequence: 0 to {len(matches_df)-1}")
            else:
                self.logger.info(f"Fixture_id column is already unique ({original_fixture_count} matches)")
            ...
            if 'date_encoded' in matches_df.columns:
                try:
                    ...
                    if matches_df['date_encoded'].dtype in ['int64', 'int32', 'float64']:
                        ...
                        matches_df['match_date'] = reference_date + pd.to_timedelta(matches_df['date_encoded'], unit='D')
                        self.logger.info("Converted date_encoded (integer days) to match_date successfully")
                        ...
                        for fmt in date_formats:
                            try:
                                ...
                                if not parsed_dates.isna().all():
                                    ...
                                    self.logger.info(f"Successfully parsed dates using format: {fmt}")
                                    break
                            except:
                                continue
                        ...
                        if matches_df['match_date'].isna().all():
                            with warnings.catch_warnings():
                                warnings.simplefilter("ignore")
                                matches_df['match_date'] = pd.to_datetime(matches_df['date_encoded'], errors='coerce')
                    ...
                    self.logger.warning(f"Date conversion failed: {str(e)}, creating sequential dates")
                    # If all else fails, create a simple date sequence
                    matches_df['match_date'] = pd.date_range(
                        start='2020-01-01', 
                        periods=len(matches_df), 
                        freq='D'
                    )
                    
                # Ensure no NaT values remain
                if matches_df['match_date'].isna().any():
                    self.logger.warning("Some dates could not be parsed, filling with sequential dates")
            ...
            if len(teams_df['team_id'].unique()) != len(teams_df):
                self.logger.warning("Duplicate team_ids detected in teams entity, removing duplicates")
            ...
            if 'venue_encoded' in matches_df.columns:
                ...
                if len(venues_df) > 0 and len(venues_df['venue_id'].unique()) != len(venues_df):
                    ...
                    venues_df = venues_df.drop_duplicates(subset=['venue_id']).reset_index(drop=True)
            ...
            if 'league_encoded' in matches_df.columns:
                ...
                if len(leagues_df) > 0 and len(leagues_df['league_id'].unique()) != len(leagues_df):
                    ...
                    leagues_df = leagues_df.drop_duplicates(subset=['league_id']).reset_index(drop=True)
            ...
            final_memory = sum(entity_df.memory_usage(deep=True).sum() for entity_df in entities.values()) / 1024**2
            memory_saved = initial_memory - final_memory
            
            self.logger.info(f"Created {len(entities)} entities for featuretools", 
                           extra={
                               'final_memory_mb': final_memory,
                               'memory_saved_mb': memory_saved,
                               'savings_percent': (memory_saved / initial_memory) * 100 if initial_memory > 0 else 0
                           })
            return entities
            
        except Exception as e:
            self.logger.error(f"Error preparing data for featuretools: {str(e)}", 
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                if "match_date" in matches_df.columns:
                    ...
                    if matches_df['match_date'].isna().any():
                        self.logger.warning("Removing rows with invalid dates for EntitySet creation")
                ...
                if 'leagues' in entities and len(entities['leagues']) > 0:
                    ...
                    if 'league_encoded' in matches_df.columns:
                        es = es.add_relationship("leagues", "league_id", "matches", "league_encoded")
    ...
    def generate_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if all(col in df.columns for col in ['home_attack_strength', 'away_defense_weakness']):
                result_df['ft_home_attack_vs_away_defense'] = (
                    df['home_attack_strength'] * (1 - df['away_defense_weakness'])
                )
                
            if all(col in df.columns for col in ['away_attack_strength', 'home_defense_weakness']):
                result_df['ft_away_attack_vs_home_defense'] = (
                    df['away_attack_strength'] * (1 - df['home_defense_weakness'])
                )
            ...
            if all(col in df.columns for col in ['home_xg_momentum', 'away_xg_momentum']):
                result_df['ft_xg_momentum_ratio'] = (
                    df['home_xg_momentum'] / (df['away_xg_momentum'] + 1e-10)
                )
            
            # League context interactions
            if all(col in df.columns for col in ['league_competitiveness', 'elo_difference']):
                result_df['ft_competitive_elo_interaction'] = (
                    df['league_competitiveness'] * abs(df['elo_difference'])
                )
    ...
    def run_hybrid_feature_engineering(
        ...
        try:
            ...
            self.logger.info("Starting optimized hybrid feature engineering pipeline", 
                           extra={
            ...
            if len(df) > self.chunk_size:
                self.logger.info(f"Dataset size ({len(df)}) exceeds chunk size ({self.chunk_size}), using chunked processing")
                return self._run_chunked_feature_engineering(df, include_temporal, include_relational, include_interactions, temporal_window, temporal_gap)
            ...
            if include_temporal:
                try:
                    ...
                    if not temporal_fm.empty:
                        ...
                        new_temporal_cols = [col for col in temporal_fm.columns if col not in df.columns]
                        if new_temporal_cols:
                            temporal_features = temporal_fm[new_temporal_cols].select_dtypes(include=[np.number])
                            
                            # Add prefix to new features
                            temporal_features.columns = [f"ft_temporal_{col}" for col in temporal_features.columns]
                            
                            result_df = result_df.join(temporal_features, how='left')
                            all_feature_defs['temporal'] = temporal_defs
                            
                            self.logger.info(f"Added {len(temporal_features.columns)} temporal features")
                        
                except Exception as e:
                    self.logger.warning(f"Temporal feature generation failed: {str(e)}")
                    self.logger.info("Skipping temporal features due to error - this is normal for small datasets or insufficient temporal data")
            ...
            if include_relational and len(entityset.dataframes) > 1:
                try:
                    ...
                    if not relational_fm.empty:
                        # Select only new numeric features
                        new_relational_cols = [col for col in relational_fm.columns if col not in df.columns]
                        if new_relational_cols:
                            relational_features = relational_fm[new_relational_cols].select_dtypes(include=[np.number])
                            
                            # Add prefix to new features
                            relational_features.columns = [f"ft_relational_{col}" for col in relational_features.columns]
                            
                            result_df = result_df.join(relational_features, how='left')
                            all_feature_defs['relational'] = relational_defs
                            
                            self.logger.info(f"Added {len(relational_features.columns)} relational features")
                        
                except Exception as e:
                    self.logger.warning(f"Relational feature generation failed: {str(e)}")
                    self.logger.info("Skipping relational features due to error - this is normal for datasets without sufficient entity relationships")
            ...
            self.logger.info(f"Hybrid feature engineering completed. Added {new_features} new features", 
                           extra={
            ...
            self.logger.error(f"Error in hybrid feature engineering: {str(e)}", 
                            extra={'function': 'run_hybrid_feature_engineering'})
            raise

    def _clean_generated_features(self, result_df: pd.DataFrame, original_df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if features_to_remove:
                ...
                self.logger.info(f"Removed {len(features_to_remove)} problematic features (constant/missing/zero)")
            else:
    ...
    def _run_chunked_feature_engineering(
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                try:
                    ...
                    chunk_result, chunk_defs = self.run_hybrid_feature_engineering(
                        chunk, include_temporal, include_relational, include_interactions, 
                        temporal_window, temporal_gap
                    )
            ...
            self.logger.info(f"Chunked processing completed. Total features: {len(result_df.columns)}")
            return result_df, all_feature_defs
    ...
    def evaluate_feature_importance(
        ...
        try:
            ...
            self.logger.info("Starting optimized feature importance evaluation", 
            ...
            if method in ['mutual_info', 'combined']:
                ...
                try:
                    ...
                    self.logger.info(f"Calculated mutual information scores for {len(new_feature_names)} features")
                except Exception as e:
            ...
            if method in ['correlation', 'combined']:
                ...
                try:
                    ...
                    self.logger.info(f"Calculated correlation scores for {len(new_feature_names)} features")
                except Exception as e:
            ...
            if method in ['variance', 'combined']:
                ...
                try:
                    ...
                    self.logger.info(f"Calculated variance scores for {len(new_feature_names)} features")
                except Exception as e:
            ...
            self.logger.info(f"Feature importance evaluation completed", 
                           extra={
                               'selected_features': len(top_features),
                               'method': method,
                               'top_feature_score': max(combined_scores.values()) if combined_scores else 0,
                               'avg_feature_score': np.mean(list(combined_scores.values())) if combined_scores else 0
                           })
            ...
            self.logger.error(f"Error in feature importance evaluation: {str(e)}", 
