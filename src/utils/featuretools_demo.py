...
def load_real_data() -> pd.DataFrame:
    ...
    combined_df = pd.concat(
        [X_train_with_target, X_test_with_target, X_val_with_target], ignore_index=True
    )
    ...
    if "date_encoded" not in combined_df.columns:
        ...
        combined_df["date_encoded"] = pd.date_range(
            start="2020-01-01", periods=len(combined_df), freq="D"
        )
    ...
    logger.info(
        f"Loaded real dataset with {len(combined_df)} rows and {len(combined_df.columns)} features"
    )
...
def demonstrate_featuretools_pipeline():
    ...
    try:
        ...
        with mlflow.start_run(run_name="hybrid_feature_engineering"):
            ...
            logger.info(
                f"   Original features: {original_feature_count} (real engineered features)"
            )
            ...
            logger.info("   Data source: Real soccer match data from DataLoader")
            ...
            if new_feature_names:
                ...
                inf_count = (
                    np.isinf(augmented_df[new_feature_names].select_dtypes(include=[np.number]))
                    .sum()
                    .sum()
                )
                if inf_count > 0:
...
def demonstrate_feature_integration():
    ...
    augmented_df, feature_defs = engineer.run_hybrid_feature_engineering(
        df=X_train, include_temporal=True, include_relational=True, include_interactions=True
    )
    ...
    if new_features:
        ...
        with open("src/utils/selected_features_ensemble_new.json") as f:
        ...
        for model_name, selected_features in model_feature_strategies.items():
            if model_name in updated_selections:
                ...
                logger.info(
                    f"Added {len(new_model_features)} strategically selected features to {model_name}"
                )
        ...
        all_top_features = list(
            dict.fromkeys(top_features_combined)
        )  # Remove duplicates while preserving order
        ...
        logger.info(
            f"Selection efficiency: {len(all_top_features)}/{len(new_features)} = {len(all_top_features) / len(new_features) * 100:.1f}%"
        )
            ...
            if "selected_" in feature_type:
                ...
                if original_type in feature_analysis:
                    ...
                    if original_count > 0:
                        ...
                        logger.info(
                            f"   {feature_type}: {selected_count}/0 (no original features of this type)"
                        )
