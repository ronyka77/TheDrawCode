    ...
    def sync_with_shared(self) -> None:
        ...
        try:
            ...
            for experiment_directory in shared_path.iterdir():
                if not experiment_directory.is_dir() or experiment_directory.name in [
                    ".trash",
                    "metadata",
                ]:
                    continue
                ...
                for run_directory in experiment_directory.iterdir():
                    ...
                    self.logger.info(
                        f"Synced run: {run_id} from experiment: {experiment_directory.name}"
                    )
        ...
        try:
            ...
            for experiment_directory in self.config.local_path.iterdir():
                ...
                for run_directory in experiment_directory.iterdir():
                    ...
                    shutil.copytree(run_directory, target_directory)
                    self.logger.info(
                        f"Backed up run: {run_id} from experiment: {experiment_directory.name}"
                    )
