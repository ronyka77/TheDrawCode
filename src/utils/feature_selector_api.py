...
class XGBoostFeatureSelector:
    ...
    def select_features(
        self, X: pd.DataFrame, y: pd.Series, X_test: pd.DataFrame, y_test: pd.Series
    ) -> list[str]:
        ...
        n_features = max(
            self.min_features, int(len(importance_df[importance_df["composite_score"] > 0]) * 0.8)
        )
        ...
        self.logger.info(
            f"Features with non-zero importance: {len(importance_df[importance_df['composite_score'] > 0])}"
        )
...
def load_data_without_selected_columns_api(
    data_path: str,
    ...
    try:
        ...
        data.drop(
            columns=["match_outcome", "home_goals", "away_goals", "Home", "Away"], inplace=True
        )
