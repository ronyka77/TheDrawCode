...
def remove_outliers_isolation_forest(
    ...
    try:
        ...
        logger.info(
            f"Detected {outlier_count} outliers ({outlier_percentage:.2f}% of training data)"
        )
        ...
        logger.info(
            f"Class distribution - Before: {original_positive_rate:.3f}, After: {new_positive_rate:.3f}"
        )
...
def analyze_outlier_impact(
    ...
    analysis["removal_percentage"] = (
        analysis["samples_removed"] / analysis["samples_before"]
    ) * 100

    # Class distribution
    analysis["positive_rate_before"] = y_before.mean()
    analysis["positive_rate_after"] = y_after.mean()
    analysis["class_distribution_change"] = abs(
        analysis["positive_rate_after"] - analysis["positive_rate_before"]
    )
    ...
    logger.info(
        f"  Samples removed: {analysis['samples_removed']} ({analysis['removal_percentage']:.2f}%)"
    )
    logger.info(f"  Class distribution change: {analysis['class_distribution_change']:.4f}")
    logger.info(
        f"  Positive rate: {analysis['positive_rate_before']:.3f} -> {analysis['positive_rate_after']:.3f}"
    )
