...
class BayesianMetaLearner:
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None):
        ...
        with pm.Model() as model:
            # Priors for weights
            weights = pm.Normal(
                "weights", mu=0, sigma=self.hyperparams["prior_scale"], shape=n_predictors
            )
            ...
            pm.Bernoulli("likelihood", p=pm.math.sigmoid(linear_pred), observed=y_train)
    ...
    def evaluate(self, X_val, y_val, threshold=0.5):
        ...
        # Log metrics
        logger.info(
            f"Bayesian Meta Learner Evaluation: accuracy={accuracy:.4f}, "
...
def optimize_hyperparameters(X_train, y_train, X_val, y_val, n_trials=20, timeout=3600):
    ...
    """
    logger.info(
        f"Starting Bayesian Meta Learner hyperparameter optimization with {n_trials} trials"
    )

    # Create study object
    study = optuna.create_study(
        direction="minimize", sampler=TPESampler(consider_prior=True, prior_weight=0.3)
    )
...
def train_with_optimal_parameters(X_train, y_train, X_val=None, y_val=None, params=None):
    ...
    # Register model
    f"bayesian_meta_ensemble_{datetime.now().strftime('%Y%m%d_%H%M')}"
...
def optimize_threshold(self, X_val, y_val, target_precision=0.5, min_recall=0.25, logger=None):
    ...
    if logger is None:
        logger = ExperimentLogger(
            experiment_name="bayesian_meta_learner", log_dir="./logs/bayesian_meta_learner"
        )
