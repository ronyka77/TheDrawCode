...
# Local imports
# Module imports
from models.ensemble.calibration import analyze_calibration, calibrate_models
from models.ensemble.data_utils import prepare_data
from models.ensemble.diagnostics import (
...
class EnsembleModel(BaseEstimator, ClassifierMixin):
    ...
    def train(
        ...
        X_train_prepared_cat = X_train_prepared[self.cat_features]
        ...
        if self.extra_base_model_type in ["mlp", "svm"] and self.extra_model_scaler is not None:
            ...
            p_extra = extra_model.predict_proba(X_val_prepared_rf)[:, 1]
            p_extra_train = extra_model.predict_proba(X_combined_rf)[:, 1]

        p_xgb = xgb_model.predict_proba(X_val_prepared_xgb)[:, 1]
        p_xgb_train = xgb_model.predict_proba(X_combined_xgb)[:, 1]

        p_cat = cat_model.predict_proba(X_val_prepared_cat)[:, 1]
        p_cat_train = cat_model.predict_proba(X_combined_cat)[:, 1]

        p_lgb = lgb_model.predict_proba(X_val_prepared_lgb)[:, 1]
        p_lgb_train = lgb_model.predict_proba(X_combined_lgb)[:, 1]
