...
def compute_dynamic_weights(
    ...
    logger.info(
        f"CatBoost: threshold={cat_threshold:.3f}, precision={cat_precision:.4f}, recall={cat_recall:.4f}, f1={cat_f1:.4f}"
    )
    logger.info(
...
def compute_precision_focused_weights(
    p_xgb, p_cat, p_lgb, p_extra, y_true, target_precision, required_recall, logger=None
):
    """
    ...
    xgb_threshold, xgb_metrics = tune_threshold_for_precision(
        p_xgb, y_true, target_precision, required_recall
    )
    cat_threshold, cat_metrics = tune_threshold_for_precision(
        p_cat, y_true, target_precision, required_recall
    )
    lgb_threshold, lgb_metrics = tune_threshold_for_precision(
        p_lgb, y_true, target_precision, required_recall
    )
    extra_threshold, extra_metrics = tune_threshold_for_precision(
