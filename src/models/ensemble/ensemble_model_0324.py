...
class EnsembleModel(BaseEstimator, ClassifierMixin):
    ...
    def train(
        ...
        X_train_prepared[self.xgb_features]
        X_train_prepared[self.tabnet_features]
        X_train_prepared[self.lgb_features]
        X_train_prepared[self.rf_features]
    ...
    def predict_proba(self, X) -> np.ndarray:
        ...
        try:
            ...
            meta_features = create_meta_features_optimized(
                ...
                p_extra,
                self.dynamic_weights if self.dynamic_weighting else None,
                self.thresholds if self.thresholds else None,
            )
    ...
    def load_models_from_mlflow(
        self, xgb_path="model", lgb_path="model", tabnet_path="model", rf_path="model"
    ):
        """
        ...
        try:
            ...
            if xgb_pyfunc.metadata.signature and xgb_pyfunc.metadata.signature.inputs:
                ...
                if hasattr(self.model_xgb, "feature_names"):
                    ...
                    self.logger.info(
                        f"Retrieved {len(self.xgb_features)} feature names directly from XGBoost model"
                    )
        except Exception as e:
        ...
        try:
            ...
            if lgb_pyfunc.metadata.signature and lgb_pyfunc.metadata.signature.inputs:
                ...
                if hasattr(self.model_lgb, "feature_name_"):
                    ...
                    self.logger.info(
                        f"Retrieved {len(self.lgb_features)} feature names directly from LightGBM model"
                    )
        except Exception as e:
        ...
        try:
            ...
            if tabnet_pyfunc.metadata.signature and tabnet_pyfunc.metadata.signature.inputs:
                ...
                if hasattr(self.model_tabnet, "feature_names_in_"):
                    ...
                    self.logger.info(
                        f"Retrieved {len(self.tabnet_features)} feature names directly from TabNet model"
                    )
                elif hasattr(self.model_tabnet, "input_dim"):
                    feature_count = self.model_tabnet.input_dim
                    self.logger.warning(
                        f"Using generic feature names for TabNet ({feature_count} features)"
                    )
        ...
        try:
            ...
            if rf_pyfunc.metadata.signature and rf_pyfunc.metadata.signature.inputs:
                ...
                if hasattr(self.model_extra, "feature_names_in_"):
                    ...
                    self.logger.info(
                        f"Retrieved {len(self.rf_features)} feature names directly from Random Forest model"
                    )
        except Exception as e:
        ...
        self.selected_features = (
            self.xgb_features
        )  # Use XGBoost features as default selected features
        return True
