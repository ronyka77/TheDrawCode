...
def evaluate_model(
    ...
    if logger is None:
        logger = ExperimentLogger(
            experiment_name="ensemble_model_evaluation", log_dir="./logs/ensemble_model_evaluation"
        )
    ...
    if threshold is None:
        if hasattr(model, "optimal_threshold"):
            ...
            threshold, metrics = tune_threshold_for_precision_optimized(
                model, X_val, y_val, logger=logger
            )
    ...
    logger.info(
        f"  Accuracy: {accuracy:.4f} (95% CI: {bootstrap_metrics['accuracy_ci'][0]:.4f}-{bootstrap_metrics['accuracy_ci'][1]:.4f})"
    )
    logger.info(
        f"  Precision: {precision:.4f} (95% CI: {bootstrap_metrics['precision_ci'][0]:.4f}-{bootstrap_metrics['precision_ci'][1]:.4f})"
    )
    logger.info(
        f"  Recall: {recall:.4f} (95% CI: {bootstrap_metrics['recall_ci'][0]:.4f}-{bootstrap_metrics['recall_ci'][1]:.4f})"
    )
    logger.info(
        f"  F1 Score: {f1:.4f} (95% CI: {bootstrap_metrics['f1_ci'][0]:.4f}-{bootstrap_metrics['f1_ci'][1]:.4f})"
    )
    logger.info(
        f"  ROC AUC: {roc_auc:.4f} (95% CI: {bootstrap_metrics['roc_auc_ci'][0]:.4f}-{bootstrap_metrics['roc_auc_ci'][1]:.4f})"
    )
...
def calculate_bootstrap_metrics(
    y_true: np.ndarray, y_prob: np.ndarray, threshold: float, n_bootstrap: int = 1000
) -> dict:
    """
...
def cross_validate(
    ...
    **model_params,
    ...
    if logger is None:
        logger = ExperimentLogger(
            experiment_name="ensemble_model_cv", log_dir="./logs/ensemble_model_cv"
        )
    ...
    for i, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        model.train(
            X_train_fold, y_train_fold, X_val=X_test_fold, y_val=y_test_fold, split_validation=False
        )
    ...
    f1_ci = (mean_f1 - t_value * std_f1 / np.sqrt(n), mean_f1 + t_value * std_f1 / np.sqrt(n))
    ...
    logger.info(
        f"  Accuracy: {mean_accuracy:.4f} ± {std_accuracy:.4f} (95% CI: {accuracy_ci[0]:.4f}-{accuracy_ci[1]:.4f})"
    )
    logger.info(
        f"  Precision: {mean_precision:.4f} ± {std_precision:.4f} (95% CI: {precision_ci[0]:.4f}-{precision_ci[1]:.4f})"
    )
    logger.info(
        f"  Recall: {mean_recall:.4f} ± {std_recall:.4f} (95% CI: {recall_ci[0]:.4f}-{recall_ci[1]:.4f})"
    )
    logger.info(f"  F1 Score: {mean_f1:.4f} ± {std_f1:.4f} (95% CI: {f1_ci[0]:.4f}-{f1_ci[1]:.4f})")
    logger.info(
        f"  ROC AUC: {mean_roc_auc:.4f} ± {std_roc_auc:.4f} (95% CI: {roc_auc_ci[0]:.4f}-{roc_auc_ci[1]:.4f})"
    )
    ...
    return {
        "metrics": {
            ...
            "recall": {"values": recalls, "mean": mean_recall, "std": std_recall, "ci": recall_ci},
            "f1_score": {"values": f1_scores, "mean": mean_f1, "std": std_f1, "ci": f1_ci},
            "roc_auc": {
                "values": roc_aucs,
                "mean": mean_roc_auc,
                "std": std_roc_auc,
            ...
            "threshold": {"values": thresholds, "mean": mean_threshold, "std": std_threshold},
        },
        "n_splits": n_splits,
    }
