...
def explain_predictions(model, X_val: pd.DataFrame, logger: ExperimentLogger = None) -> dict:
    """
    ...
    if hasattr(model, "meta_learner") and model.meta_learner is not None:
        ...
        try:
            ...
            return {"error": f"Meta-feature creation failed: {str(e)}", "feature_importance": {}}
    else:
    ...
    try:
        ...
        return {"feature_importance": dict(sorted_importance), "shap_values": shap_values}
...
def analyze_prediction_errors(
    ...
    X_val.iloc[fp_indices] if len(fp_indices) > 0 else pd.DataFrame()
    X_val.iloc[fn_indices] if len(fn_indices) > 0 else pd.DataFrame()
