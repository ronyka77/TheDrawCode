...
def compute_dynamic_weights(
    ...
    if logger is None:
        logger = ExperimentLogger(
            experiment_name="ensemble_model_weights", log_dir="./logs/ensemble_model_weights"
        )
    ...
    xgb_threshold, xgb_precision, xgb_recall, xgb_f1 = tune_threshold_for_precision_optimized(
        p_xgb, targets
    )
    tabnet_threshold, tabnet_precision, tabnet_recall, tabnet_f1 = (
        tune_threshold_for_precision_optimized(p_tabnet, targets)
    )
    lgb_threshold, lgb_precision, lgb_recall, lgb_f1 = tune_threshold_for_precision_optimized(
        p_lgb, targets
    )
    extra_threshold, extra_precision, extra_recall, extra_f1 = (
        tune_threshold_for_precision_optimized(p_extra, targets)
    )

    # Log individual model metrics
    logger.info(
        f"XGBoost: threshold={xgb_threshold:.3f}, precision={xgb_precision:.4f}, recall={xgb_recall:.4f}, f1={xgb_f1:.4f}"
    )
    logger.info(
        f"TabNet: threshold={tabnet_threshold:.3f}, precision={tabnet_precision:.4f}, recall={tabnet_recall:.4f}, f1={tabnet_f1:.4f}"
    )
    logger.info(
        f"LightGBM: threshold={lgb_threshold:.3f}, precision={lgb_precision:.4f}, recall={lgb_recall:.4f}, f1={lgb_f1:.4f}"
    )
    logger.info(
        f"Extra Model: threshold={extra_threshold:.3f}, precision={extra_precision:.4f}, recall={extra_recall:.4f}, f1={extra_f1:.4f}"
    )
    ...
    for model in raw_weights:
        if raw_weights[model] < min_weight:
            ...
            remaining_raw = sum(
                [w for m, w in raw_weights.items() if min_weight <= w <= max_weight]
            )
...
def compute_precision_focused_weights_optimized(
    p_xgb, p_tabnet, p_lgb, p_extra, y_true, target_precision, required_recalls, logger=None
):
    """
...
def compute_precision_focused_weights(
    p_xgb, p_tabnet, p_lgb, p_extra, y_true, target_precision, required_recall, logger=None
):
    """
    Compute weights with strong focus on precision
    """
    ...
    # Find precision-optimal thresholds
    xgb_threshold, xgb_metrics = tune_threshold_for_precision_optimized(
        p_xgb, y_true, target_precision, required_recall
    )
    tabnet_threshold, tabnet_metrics = tune_threshold_for_precision_optimized(
        p_tabnet, y_true, target_precision, required_recall
    )
    lgb_threshold, lgb_metrics = tune_threshold_for_precision_optimized(
        p_lgb, y_true, target_precision, required_recall
    )
    extra_threshold, extra_metrics = tune_threshold_for_precision_optimized(
        p_extra, y_true, target_precision, required_recall
    )
