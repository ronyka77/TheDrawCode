...
def apply_adasyn_resampling(
    ...
    if logger is None:
        logger = ExperimentLogger(
            experiment_name="ensemble_model_data", log_dir="./logs/ensemble_model_data"
        )
    ...
    logger.info(
        f"Class distribution before ADASYN: {n_pos}/{n_samples} positive ({n_pos / n_samples:.2%})"
    )
    ...
    if min(n_pos, n_neg) < min_minority_samples:
        logger.warning(
            f"Too few minority samples ({min(n_pos, n_neg)}) for ADASYN resampling. Skipping."
        )
        return X_train, y_train
    ...
    try:
        ...
        logger.info(
            f"Class distribution after ADASYN: {n_pos_after}/{n_samples_after} positive ({n_pos_after / n_samples_after:.2%})"
        )
...
def balance_and_clean_dataset(
    ...
    """
    if logger is None:
        logger = ExperimentLogger(experiment_name="data_cleaning", log_dir="./logs/data_cleaning")
    ...
    try:
        ...
        if outlier_count > 0:
            logger.info(
                f"Removing {outlier_count} outliers ({outlier_count / len(X_clean):.2%} of data)"
            )
    ...
    logger.info(
        f"Class distribution before balancing: {n_pos}/{n_samples} positive ({n_pos / n_samples:.2%})"
    )
    ...
    try:
        ...
        logger.info(
            f"Class distribution after balancing: {n_pos_after}/{n_samples_after} positive ({n_pos_after / n_samples_after:.2%})"
        )
        logger.info(f"Added {n_samples_after - n_samples} synthetic samples")
...
def select_features_by_importance(
    ...
    if logger is None:
        logger = ExperimentLogger(
            experiment_name="ensemble_model_feature_selection",
    ...
    selected_features = [
        col for col, importance in sorted_features if importance >= importance_threshold
    ]
