...
def calibrate_models(
    ...
    def calibrate_model(model, model_name, scaler=None):
        ...
        if n_splits < 2 or min_class_samples < 2 * min_samples_per_fold:
            logger.warning(
                f"Not enough samples for cross-validation calibration for {model_name}. Using 'prefit' mode."
            )
            cv_strategy = "prefit"
        ...
        scaler.transform(X_test_prepared) if scaler is not None else X_test_prepared
        X_train_prepared = (
            scaler.transform(X_train_prepared) if scaler is not None else X_train_prepared
        )
        ...
        logger.info(
            f"{model_name} Brier score - Before: {brier_before:.4f}, After: {brier_after:.4f}"
        )
    ...
    return {"calibrated_models": calibrated_models, "calibration_results": calibration_results}


def analyze_calibration(
    calibration_results: dict, y_test: pd.Series, logger: ExperimentLogger = None
) -> dict:
    """
...
def calculate_calibration_metrics(
    probabilities: np.ndarray, targets: np.ndarray, bins: list[float]
) -> dict:
    """
