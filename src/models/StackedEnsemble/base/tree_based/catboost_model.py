...
pip_requirements = [f"catboost=={cb.__version__}", f"mlflow=={mlflow.__version__}"]
...
def load_hyperparameter_space():
    ...
    try:
        ...
        hyperparameter_space = {
            ...
            "depth": {"type": "int", "low": 6, "high": 12, "step": 1},
            "min_data_in_leaf": {"type": "int", "low": 10, "high": 200, "step": 5},
            "subsample": {"type": "float", "low": 0.55, "high": 0.95, "step": 0.02},
            "colsample_bylevel": {"type": "float", "low": 0.3, "high": 0.7, "step": 0.02},
            "reg_lambda": {"type": "float", "low": 0.5, "high": 10.0, "log": True},
            "leaf_estimation_iterations": {"type": "int", "low": 2, "high": 20, "step": 2},
            "bagging_temperature": {"type": "float", "low": 1.0, "high": 10.0, "step": 0.1},
            "scale_pos_weight": {"type": "float", "low": 1.5, "high": 5.0, "step": 0.05},
            "early_stopping_rounds": {"type": "int", "low": 200, "high": 700, "step": 10},
        }
...
def optimize_hyperparameters(
    ...
    try:
        ...
        study = optuna.create_study(
            study_name="catboost_optimization", direction="maximize", sampler=random_sampler
        )
...
def hypertune_catboost(experiment_name: str):
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"catboost_base_{datetime.now().strftime('%Y%m%d_%H%M')}"):
            # Set tags
            mlflow.set_tags(
                {"model_type": "catboost_base", "training_mode": "global", "cpu_only": True}
            )
