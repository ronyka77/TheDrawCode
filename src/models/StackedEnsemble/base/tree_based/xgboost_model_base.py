...
from models.StackedEnsemble.shared.hypertuner_utils import calculate_feature_importance, evaluate
...
def train_model(X_train, y_train, X_test, y_test, X_eval, y_eval, model_params):
    ...
    try:
        ...
        model.fit(X_combined, y_combined, eval_set=eval_set, verbose=False)

        # Get predictions using fixed 0.5 threshold
        (model.predict_proba(X_eval)[:, 1] >= 0.5).astype(int)
...
def hypertune_xgboost(experiment_name: str):
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"xgboost_base_{datetime.now().strftime('%Y%m%d_%H%M')}"):
            # Set tags
            mlflow.set_tags(
                {"model_type": "xgboost_base", "training_mode": "global", "cpu_only": True}
            )
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"xgboost_{datetime.now().strftime('%Y%m%d_%H%M')}") as run:
            ...
            importance_df = calculate_feature_importance(
                model, feature_names=X_train.columns if hasattr(X_train, "columns") else None
            )
