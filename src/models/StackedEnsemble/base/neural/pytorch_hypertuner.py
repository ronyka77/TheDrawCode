...
experiment_name = "pytorch_optimization"
...
def train_pytorch_model(
    ...
    try:
        ...
        y_train_tensor = torch.tensor(y_combined.values, dtype=torch.float32).unsqueeze(
            1
        )  # Ensure (N, 1)
...
def optimize_hyperparameters(
    ...
    try:
        study.optimize(
            objective_func, n_trials=n_trials, callbacks=[log_callback], show_progress_bar=True
        )
...
def train_with_precision_target_pytorch(
    ...
    device: torch.device,
    scaler: StandardScaler,
    # fixed_params: dict # Removed: Params will be hardcoded below
):
    """
    ...
    try:
        ...
        fixed_params = {
            "learning_rate": 0.00012540204133324443,
            "optimizer": "Adam",
            "weight_decay": 1.9814244228594363e-05,
            "batch_size": 256,
            "num_epochs": 300,
            "num_layers": 1,
            "hidden_units": 224,
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Target mean - Train: {y_train.mean():.3f}, Val: {y_val.mean():.3f}, Test: {y_test.mean():.3f}"
        )
        ...
        if best_params and final_metrics:
            logger.info("HPO process completed successfully.")
            logger.info(f"Best parameters found: {best_params}")
            logger.info(f"Final model metrics: {final_metrics}")
        else:
            logger.error("HPO process failed or did not complete successfully.")
            logger.info("Running training with fixed best parameters...")
