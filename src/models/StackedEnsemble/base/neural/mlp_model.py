...
def load_hyperparameter_space():
    ...
    hyperparameter_space = {
        ...
        "dropout_rate": {"type": "float", "low": 0.1, "high": 0.6, "step": 0.05},
        "weight_decay": {"type": "float", "low": 1e-6, "high": 1e-3, "log": True},
        "batch_size": {"type": "int", "low": 256, "high": 2048, "step": 256},
        "epochs": {"type": "int", "low": 20, "high": 150},
        "patience": {"type": "int", "low": 5, "high": 25},
        "verbose": {"type": "int", "low": 0, "high": 0},
    }
...
def optimize_threshold(
    model, dataloader, min_threshold=0.2, max_threshold=0.9, step=0.01, min_recall=0.30
):
    """
    ...
    logger.info(
        f"Optimized threshold: {best_thresh:.4f} -> Precision: {best_precision:.4f}, Recall: {best_recall:.4f}, F1: {best_f1:.4f}, AUC: {best_auc:.4f}"
    )
    metrics = {
...
def train_model_pytorch(model, params, X_train_scaled, y_train, X_eval_scaled, y_eval):
    ...
    eval_dataset = TensorDataset(
        torch.tensor(X_eval_scaled, dtype=torch.float32), torch.tensor(y_eval, dtype=torch.float32)
    )
    ...
    for epoch in range(epochs):
        ...
        for _i, (inputs, labels) in enumerate(train_loader):
            ...
            )  # Ensure labels are [batch_size, 1]
        ...
        try:
            ...
            logger.warning(
                f"Epoch {epoch + 1}: Could not calculate AUC (perhaps only one class present in batch?): {e}"
            )
            val_auc = 0.0  # Or handle as appropriate
        history["val_loss"].append(avg_val_loss)
        history["val_auc"].append(val_auc)
        logger.info(
            f"Epoch {epoch + 1}/{epochs} - Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, Val AUC: {val_auc:.4f}"
        )
        # ------------------
        ...
        if avg_val_loss < best_val_loss:
            ...
            epochs_no_improve += 1
            logger.info(
                f"Validation loss did not improve for {epochs_no_improve} epoch(s). Patience: {patience}"
            )
...
def create_model_pytorch(input_dim, params):
    ...
    try:
        ...
        logger.debug(
            f"Created SimpleMLP model with: hidden_dims={hidden_dims}, dropout={dropout_rate}"
        )
        return model
    except Exception as e:
...
def optimize_hyperparameters_pytorch(
    scaler, input_dim, X_train, y_train, X_eval, y_eval, hyperparameter_space
):
    """
    ...
    def objective(trial):
        try:
            ...
            eval_loader = TorchDataLoader(
                eval_dataset, batch_size=suggested_params["batch_size"] * 2
            )  # Use alias and larger batch size
            ...
            logger.info(
                f"Trial {trial.number}: Score: {score:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, Threshold: {best_threshold:.4f}, Val AUC: {metrics.get('auc', 0.0):.4f}"
            )
            # -----------------------
            ...
            )  # Log epoch where training stopped
            ...
            intermediate_value = history["val_loss"][
                -1
            ]  # Use last validation loss for pruning check
            trial.report(intermediate_value, step=len(history["train_loss"]))
            if trial.should_prune():
                logger.info(
                    f"Trial {trial.number} pruned based on intermediate value: {intermediate_value:.4f}"
                )
    ...
    def callback(study, trial):
        ...
        if trial.value is not None and trial.value > best_score:
            ...
            logger.info(
                f"New best score found in trial {trial.number}: {best_score:.4f} with HPO params: {best_params_hpo}"
            )
        ...
        current_run = (
            ...
            trial.params,
            trial.number,
            trial.user_attrs.get("eval_precision", 0.0),
            trial.user_attrs.get("eval_recall", 0.0),
        )
        ...
        if trial.number % 9 == 0:
            ...
            table_separator = "|------|---------|--------|-----------|--------|------------|"
            table_rows = [
                f"| {i + 1:<4} | {rec[2]:<7} | {rec[0]:<6.4f} | {rec[3]:<9.4f} | {rec[4]:<6.4f} | {rec[1]} |"
    ...
    for batch in range(num_batches):
        ...
        logger.info(
            f"Starting Optuna batch {batch + 1}/{num_batches} with sampler (seed={random_seed})"
        )
        study.optimize(
            objective,
    ...
    if global_top_trials and global_top_trials[0][0] is not None and global_top_trials[0][0] > -1.0:
        ...
        logger.warning(
            "No successful trials found meeting criteria or best score was <= 0. Cannot determine best HPO parameters."
        )
...
def hypertune_mlp_pytorch(experiment_name: str):
    ...
    try:
        ...
        with mlflow.start_run(
            run_name=f"mlp_pytorch_base_{datetime.now().strftime('%Y%m%d_%H%M')}"
        ):
            mlflow.set_tags(
                {
            ...
            if not best_hpo_params:
                logger.error(
                    "Hyperparameter optimization failed to find suitable parameters. Aborting."
                )
                return None, None
            ...
            final_model = create_model_pytorch(
                input_dim=input_dim, params=best_hpo_params
            )  # Pass full dict
            # --------------------------------
            ...
            final_eval_dataset = TensorDataset(
                torch.tensor(X_eval_final_scaled, dtype=torch.float32),
                torch.tensor(y_eval, dtype=torch.float32),
            )
            ...
            final_eval_loader = TorchDataLoader(
                final_eval_dataset, batch_size=final_batch_size
            )  # Use alias
            # Pass the correctly created TorchDataLoader instance
            final_threshold, final_metrics = optimize_threshold(
                final_model, final_eval_loader, min_recall=min_recall
            )
...
def main():
    ...
    try:
        ...
        X_train, y_train, X_test, y_test, X_eval, y_eval = (
            data_loader_instance.load_data()
        )  # Use instance
        ...
        try:
            ...
            logger.warning(
                "MLP-specific features not found. Falling back to 'xgb' features."
            )  # Example fallback
        ...
        logger.info(
            f"Data shapes after feature selection - Train: {X_train.shape}, Test: {X_test.shape}, Eval: {X_eval.shape}"
        )
        logger.info(
        ...
        if best_hpo_params and final_metrics:
            logger.info("Hypertuning and final model training completed successfully.")
