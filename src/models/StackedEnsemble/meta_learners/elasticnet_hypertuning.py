...
try:
    ...
    raise ImportError(f"Error importing modules: {str(e)}") from e
...
def create_param_space(trial):
    ...
    base_alpha = trial.suggest_float(
        "base_alpha", 1e-3, 5e-2, log=True
    )  # Narrowed range for more stable convergence
    alpha_grid_size = trial.suggest_int(
        "alpha_grid_size", 150, 250
    )  # Increased minimum for better granularity
    ...
    params = {
        ...
        "positive": trial.suggest_categorical(
            "positive", [False, True]
        ),  # Prioritize unconstrained coefficients
...
def run_elasticnet_hypertuning():
    ...
    study.optimize(
        lambda trial: objective(
            trial, X_train_scaled, y_train, X_val_scaled, y_val, X_test_scaled, y_test
        ),
        n_trials=200,
        timeout=3600,
        callbacks=[lambda study, trial: log_trial_status(study, trial, exp_logger)],
    )
    ...
    best_metrics = model.fit(
        X_train_scaled, y_train, X_val_scaled, y_val, X_test_scaled, y_test, **study.best_params
    )
...
def objective(trial, X_train_scaled, y_train, X_val_scaled, y_val, X_test_scaled, y_test):
    ...
    try:
        ...
        try:
            metrics = model.fit(
                X_train_scaled, y_train, X_val_scaled, y_val, X_test_scaled, y_test, **params
            )
