...
class ElasticNetModel:
    ...
    def __init__(
        ...
        self.model = None
        self.scaler = (
            RobustScaler(quantile_range=(5, 95)) if scaling_method == "robust" else StandardScaler()
        )
    ...
    def _select_features(self, X: pd.DataFrame, y: pd.Series) -> list[str]:
        ...
        lasso = LassoCV(cv=3, random_state=42, max_iter=10000, tol=1e-4, n_jobs=-1)
        lasso.fit(X, y)
    ...
    def train(
        self, X: Any, y: Any, X_val: Any, y_val: Any, X_test: Any, y_test: Any, **kwargs
    ) -> dict[str, float]:
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"elasticnet_{datetime.now().strftime('%Y%m%d_%H%M')}"):
            try:
                ...
                # Set MLflow tags
                mlflow.set_tags(
                    {"model_type": "elasticnet", "training_mode": "global", "cpu_only": True}
                )
                params = {
                ...
                while precision < target_precision:  # Lower target for ElasticNet
                    for random_seed in range(1, 600):
                        ...
                        metrics = elasticnet_model.fit(
                            X_train_scaled, y_train, X_test_scaled, y_test, X_eval_scaled, y_eval
                        )
                        ...
                        if precision >= target_precision:
                            ...
                            break
                        logger.info(
                            f"Current precision: {precision:.4f}, target: {target_precision:.4f} highest precision: {highest_precision:.4f} best seed: {best_seed}"
                        )
                    if precision < target_precision:
