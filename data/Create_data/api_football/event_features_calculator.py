    ...
    for col in required_cols:
        if col not in events_df.columns:
            ...
            if col in [
                ...
                "away_team_id",
                "team_id",
            ]:  # These are IDs, float is ok for NaN
...
def calculate_event_features_vectorized(
    events_df: pd.DataFrame, fixtures_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    if not all(
        col in fixtures_df.columns for col in ["fixture_id", "home_team_id", "away_team_id"]
    ):
        raise ValueError(
            "fixtures_df must contain 'fixture_id', 'home_team_id', and 'away_team_id'."
        )

    # Merge home/away team_ids from fixtures_df to events_df to ensure they are present and correct
    events_df = pd.merge(
        events_df.drop(columns=["home_team_id", "away_team_id"], errors="ignore"),
    ...
    if (
        processed_events_df.empty and not events_df.empty
    ):  # Preprocessing made it empty, likely due to missing critical columns
        print(
            "Warning: processed_events_df became empty after preprocessing. Check input events_df columns."
        )
        return pd.DataFrame(columns=["fixture_id"] + get_feature_names())
    ...
    for col in expected_cols:
        if col not in features_df.columns:
            ...
            if (
                col.startswith("is_")
                or col.startswith("late_equalizer")
                or col.startswith("red_card")
                or col.startswith("low_total")
                or col.startswith("first_goal_")
            ):
                features_df[col] = 0
...
def _get_score_at_time_vectorized(
    processed_events_df: pd.DataFrame, time_limit: int, fixtures_info_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    events_at_time_limit = processed_events_df[
        processed_events_df["time_elapsed"] <= time_limit
    ].copy()
    ...
    events_at_time_limit["is_home_goal"] = (events_at_time_limit["is_goal"]) & (
        events_at_time_limit["team_id"] == events_at_time_limit["home_team_id"]
    )
    events_at_time_limit["is_away_goal"] = (events_at_time_limit["is_goal"]) & (
        events_at_time_limit["team_id"] == events_at_time_limit["away_team_id"]
    )

    scores_at_time = (
        events_at_time_limit.groupby("fixture_id")
        .agg(home_goals_at_time=("is_home_goal", "sum"), away_goals_at_time=("is_away_goal", "sum"))
        .reset_index()
    )
    ...
    all_fixtures_scores["home_goals_at_time"] = all_fixtures_scores["home_goals_at_time"].astype(
        int
    )
    all_fixtures_scores["away_goals_at_time"] = all_fixtures_scores["away_goals_at_time"].astype(
        int
    )
...
def _calculate_is_score_tied_at_80_min_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    is_tied_series = (
        scores_at_80_min["home_goals_at_time"] == scores_at_80_min["away_goals_at_time"]
    ).astype(int)
    is_tied_series.name = "is_score_tied_at_80_min"
    return is_tied_series


def _calculate_abs_goal_difference_at_45_min_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    abs_diff_series = (
        (scores_at_45_min["home_goals_at_time"] - scores_at_45_min["away_goals_at_time"])
        .abs()
        .astype(int)
    )
...
def _late_equalizer_check_fixture(fixture_events: pd.DataFrame) -> int:
    ...
    late_goals = fixture_events[
        (fixture_events["is_goal"]) & (fixture_events["time_elapsed"] >= 75)
    ].copy()
    ...
    fixture_events_sorted["is_home_goal_event"] = (fixture_events_sorted["is_goal"]) & (
        fixture_events_sorted["team_id"] == home_team_id
    )
    fixture_events_sorted["is_away_goal_event"] = (fixture_events_sorted["is_goal"]) & (
        fixture_events_sorted["team_id"] == away_team_id
    )

    fixture_events_sorted["current_home_score"] = fixture_events_sorted[
        "is_home_goal_event"
    ].cumsum()
    fixture_events_sorted["current_away_score"] = fixture_events_sorted[
        "is_away_goal_event"
    ].cumsum()
    ...
    for _, goal_event in late_goals.iterrows():
        ...
        events_before_goal = fixture_events_sorted[
            fixture_events_sorted["time_elapsed"] < time_of_goal
        ]
        ...
        if (
            home_goals_after_event == away_goals_after_event
            and home_goals_before_event != away_goals_before_event
        ):
            return 1
    return 0
...
def _calculate_late_equalizer_scored_in_last_15_mins_vectorized(
    ...
    if processed_events_df.empty:
        result_series = pd.Series(
            0,
            index=fixtures_info_df["fixture_id"].unique(),
            name="late_equalizer_scored_in_last_15_mins",
        )
    ...
    if not all(col in processed_events_df.columns for col in ["home_team_id", "away_team_id"]):
        raise ValueError(
            "processed_events_df must contain 'home_team_id' and 'away_team_id' for late equalizer check."
        )
...
def _red_card_leading_team_check_fixture(fixture_events: pd.DataFrame) -> int:
    ...
    for _, card_event in red_card_events_late.iterrows():
        ...
        events_before_card = fixture_events_sorted[
            fixture_events_sorted["time_elapsed"] < time_of_card
        ]
...
def _calculate_red_card_to_leading_team_after_60_min_vectorized(
    ...
    if processed_events_df.empty:
        result_series = pd.Series(
            0,
            index=fixtures_info_df["fixture_id"].unique(),
            name="red_card_to_leading_team_after_60_min",
        )
        return result_series.reindex(fixtures_info_df.set_index("fixture_id").index).fillna(0)

    if not all(
        col in processed_events_df.columns
        for col in ["home_team_id", "away_team_id", "event_detail"]
    ):
        # If event_detail is missing, this feature cannot be calculated as specified.
        print(
            "Warning: 'home_team_id', 'away_team_id', or 'event_detail' missing in processed_events_df for red card check. Returning 0s."
        )
        result_series = pd.Series(
            0,
            index=fixtures_info_df["fixture_id"].unique(),
        ...
        return (
            result_series.reindex(fixtures_info_df.set_index("fixture_id").index)
            .fillna(0)
            .astype(int)
        )

    red_card_series = processed_events_df.groupby("fixture_id").apply(
        _red_card_leading_team_check_fixture, include_groups=False
    )
...
def _max_continuous_time_tied_fixture(fixture_events: pd.DataFrame) -> float:
    ...
    if not fixture_events.empty:
        ...
        if (
            max_event_time > match_end_time
        ):  # Handle cases where events (like goals) are recorded past 90 min
    ...
    _fixture_events_sorted["is_home_g"] = (_fixture_events_sorted["is_goal"]) & (
        _fixture_events_sorted["team_id"] == home_team_id
    )
    _fixture_events_sorted["is_away_g"] = (_fixture_events_sorted["is_goal"]) & (
        _fixture_events_sorted["team_id"] == away_team_id
    )
    ...
    def get_score_at_t(t_limit, _events_with_cum_scores):
        relevant_events = _events_with_cum_scores[
            _events_with_cum_scores["time_elapsed"] <= t_limit
        ]
    ...
    for i in range(len(time_points)):
        ...
        time_points[i - 1] if i > 0 else 0.0
        ...
        h_goals_at_current_t, a_goals_at_current_t = get_score_at_t(
            current_segment_end_time, _fixture_events_sorted
        )

        if score_is_currently_tied:
            if (
                h_goals_at_current_t != a_goals_at_current_t
            ):  # Score just became un-tied at current_segment_end_time
            ...
            if (
                h_goals_at_current_t == a_goals_at_current_t
            ):  # Score just became tied at current_segment_end_time
                score_is_currently_tied = True
                current_tied_streak_start_time = (
                    current_segment_end_time  # New tied streak starts now
                )
...
def _calculate_max_continuous_time_score_tied_overall_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty:
        ...
        result_series = pd.Series(
            90.0,
            index=fixtures_info_df["fixture_id"].unique(),
            name="max_continuous_time_score_tied_overall",
        )
    ...
    if not all(col in processed_events_df.columns for col in ["home_team_id", "away_team_id"]):
        ...
        print(
            "Warning: 'home_team_id' or 'away_team_id' missing for max_continuous_time_score_tied. Returning 0s."
        )
        result_series = pd.Series(
            0.0,
            index=fixtures_info_df["fixture_id"].unique(),
    ...
    max_tied_time_series = processed_events_df.groupby("fixture_id").apply(
        _max_continuous_time_tied_fixture, include_groups=False
    )
    max_tied_time_series.name = "max_continuous_time_score_tied_overall"

    base_fixture_ids = fixtures_info_df.set_index("fixture_id").index
    result_series = max_tied_time_series.reindex(base_fixture_ids).fillna(
        90.0
    )  # Default for missing fixtures (no events)

    return result_series


def _calculate_low_total_goals_match_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty:
        ...
        result_series = pd.Series(
            1, index=fixtures_info_df["fixture_id"].unique(), name="low_total_goals_match"
        )
        return (
            result_series.reindex(fixtures_info_df.set_index("fixture_id").index)
            .fillna(1)
    ...
    base_fixture_ids = fixtures_info_df.set_index("fixture_id").index
    result_series = (
        low_goals_series.reindex(base_fixture_ids).fillna(1).astype(int)
    )  # Default to 1 (0 goals)

    return result_series


def _calculate_first_goal_scored_late_or_no_goals_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty:
        ...
        result_series = pd.Series(
            1,
            index=fixtures_info_df["fixture_id"].unique(),
            name="first_goal_scored_late_or_no_goals",
        )
        return (
            result_series.reindex(fixtures_info_df.set_index("fixture_id").index)
            .fillna(1)
    ...
    base_fixture_ids = fixtures_info_df.set_index("fixture_id").index
    result_series = (
        late_first_goal_series.reindex(base_fixture_ids).fillna(1).astype(int)
    )  # Default to 1 (no goals implies condition met)
...
def _late_subs_intensity_check_fixture(fixture_events: pd.DataFrame) -> int:
    ...
    late_subs = fixture_events[
        (fixture_events["event_type"] == "subst") & (fixture_events["time_elapsed"] >= 70)
    ].copy()
    ...
    for _, sub_event in late_subs.iterrows():
        ...
        home_goals_before_sub, away_goals_before_sub = get_score_just_before_t(
            time_of_sub, _fixture_events_sorted
        )
...
def _calculate_late_substitutions_intensity_when_close_vectorized(
    ...
    if processed_events_df.empty or "subst" not in processed_events_df["event_type"].unique():
        result_series = pd.Series(
            0,
            index=fixtures_info_df["fixture_id"].unique(),
            name="late_substitutions_intensity_when_close",
        )
        return (
            result_series.reindex(fixtures_info_df.set_index("fixture_id").index)
            .fillna(0)
    ...
    if not all(
        col in processed_events_df.columns for col in ["home_team_id", "away_team_id", "event_type"]
    ):
    ...
    intensity_series = processed_events_df.groupby("fixture_id").apply(
...
def _rolling_avg_goal_diff_last_30_fixture(fixture_events: pd.DataFrame) -> float:
    ...
    def get_score_at_minute_t(minute_val, _events_with_cum_scores):
        relevant_events = _events_with_cum_scores[
            _events_with_cum_scores["time_elapsed"] <= minute_val
        ]
        if relevant_events.empty:
            return 0, 0
        last_event_at_t = relevant_events.iloc[-1]
...
def _calculate_rolling_avg_goal_difference_last_30_min_at_end_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty:
        result_series = pd.Series(
            0.0,
            index=fixtures_info_df["fixture_id"].unique(),
            name="rolling_avg_goal_difference_last_30_min_at_end",
        )
        return result_series.reindex(fixtures_info_df.set_index("fixture_id").index).fillna(0.0)
    ...
    avg_goal_diff_series = processed_events_df.groupby("fixture_id").apply(
    ...
    base_fixture_ids = fixtures_info_df.set_index("fixture_id").index
    result_series = avg_goal_diff_series.reindex(base_fixture_ids).fillna(
        0.0
    )  # Default for no events is 0.0 diff
    return result_series
...
def _calculate_mean_time_between_goals_overall_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty:
        result_series = pd.Series(
            np.nan,
            index=fixtures_info_df["fixture_id"].unique(),
            name="mean_time_between_goals_overall",
        )
    ...
    mean_time_series = goal_events_sorted.groupby("fixture_id").apply(
        _mean_time_between_goals_fixture, include_groups=False
    )
    mean_time_series.name = "mean_time_between_goals_overall"

    base_fixture_ids = fixtures_info_df.set_index("fixture_id").index
    result_series = mean_time_series.reindex(base_fixture_ids).fillna(
        np.nan
    )  # Default for no/few goals is NaN
    return result_series


def _calculate_abs_shots_on_target_diff_last_20_min_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty:
        result_series = pd.Series(
            0,
            index=fixtures_info_df["fixture_id"].unique(),
            name="abs_shots_on_target_diff_last_20_min",
        )
        return (
            result_series.reindex(fixtures_info_df.set_index("fixture_id").index)
            .fillna(0)
    ...
    sot_condition = (
        (
            (processed_events_df["event_type"] == "Shot")
            & (processed_events_df["event_detail"] == "Shot on Target")
        )
        | (processed_events_df["event_detail"] == "Shot on target")  # common variation
        | (
            processed_events_df["event_detail"] == "On Target"
        )  # common variation for event_type 'shot'
    )
    ...
    sot_counts = late_sot_events.groupby("fixture_id").agg(
        home_sots_late=("is_home_sot", "sum"), away_sots_late=("is_away_sot", "sum")
    )

    abs_diff_sot_series = (
        (sot_counts["home_sots_late"] - sot_counts["away_sots_late"]).abs().astype(int)
    )
...
def _calculate_total_yellow_cards_first_half_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty or "event_detail" not in processed_events_df.columns:
        result_series = pd.Series(
            0, index=fixtures_info_df["fixture_id"].unique(), name="total_yellow_cards_first_half"
        )
        return (
            result_series.reindex(fixtures_info_df.set_index("fixture_id").index)
            .fillna(0)
...
def _calculate_abs_goal_diff_0_30_min_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    abs_diff_series = (
        (scores_at_30_min["home_goals_at_time"] - scores_at_30_min["away_goals_at_time"])
        .abs()
        .astype(int)
    )
...
def _calculate_abs_goal_diff_30_60_min_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    home_goals_in_period = (
        scores_at_60_min["home_goals_at_time"] - scores_at_30_min["home_goals_at_time"]
    )
    away_goals_in_period = (
        scores_at_60_min["away_goals_at_time"] - scores_at_30_min["away_goals_at_time"]
    )
...
def _calculate_abs_goal_diff_60_90_min_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    home_goals_in_period = (
        scores_at_90_min["home_goals_at_time"] - scores_at_60_min["home_goals_at_time"]
    )
    away_goals_in_period = (
        scores_at_90_min["away_goals_at_time"] - scores_at_60_min["away_goals_at_time"]
    )
...
def _calculate_abs_substitutions_diff_overall_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty or "subst" not in processed_events_df["event_type"].unique():
        result_series = pd.Series(
            0, index=fixtures_info_df["fixture_id"].unique(), name="abs_substitutions_diff_overall"
        )
        return (
            result_series.reindex(fixtures_info_df.set_index("fixture_id").index)
            .fillna(0)
    ...
    if not all(col in sub_events.columns for col in ["home_team_id", "away_team_id"]):
        ...
        print(
            "Warning: home_team_id or away_team_id missing in sub_events. Cannot calculate abs_substitutions_diff_overall accurately. Returning 0s."
        )
        result_series = pd.Series(
            0, index=fixtures_info_df["fixture_id"].unique(), name="abs_substitutions_diff_overall"
        )
    ...
    sub_counts = sub_events.groupby("fixture_id").agg(
        home_subs=("is_home_sub", "sum"), away_subs=("is_away_sub", "sum")
    )
...
def _proportion_time_tied_fixture(fixture_events: pd.DataFrame) -> float:
    ...
    if not time_points or match_end_time == 0:  # No duration or invalid match end time
        ...
        if match_end_time == 0:
            ...
            if not fixture_events[fixture_events["time_elapsed"] == 0].empty:
                goals_at_0 = fixture_events[
                    (fixture_events["time_elapsed"] == 0) & (fixture_events["is_goal"])
                ].copy()
    ...
    for i in range(len(time_points) - 1):  # Iterate up to the second to last point
        ...
        h_goals_at_segment_start, a_goals_at_segment_start = get_score_at_t_for_proportion(
            segment_start_time, _fixture_events_sorted
        )
    ...
    return (
        round(total_time_score_tied / match_end_time, 3)
        if match_end_time > 0
        else (1.0 if total_time_score_tied == 0 else 0.0)
    )  # Avoid div by zero, though match_end_time logic tries to prevent it.
...
def _calculate_proportion_of_game_time_score_is_tied_at_end_vectorized(
    processed_events_df: pd.DataFrame, fixtures_info_df: pd.DataFrame
) -> pd.Series:
    """
    ...
    if processed_events_df.empty:
        result_series = pd.Series(
            1.0,
            index=fixtures_info_df["fixture_id"].unique(),
            name="proportion_of_game_time_score_is_tied_at_end",
        )
        return result_series.reindex(fixtures_info_df.set_index("fixture_id").index).fillna(1.0)

    if not all(col in processed_events_df.columns for col in ["home_team_id", "away_team_id"]):
        print(
            "Warning: Essential columns missing for proportion_of_game_time_score_is_tied. Returning NaNs or 0s might be better here."
        )
    ...
    try:
        proportion_series = processed_events_df.groupby("fixture_id").apply(
            lambda x: _proportion_time_tied_fixture(x)
        )
        proportion_series.name = "proportion_of_game_time_score_is_tied_at_end"

        base_fixture_ids = fixtures_info_df.set_index("fixture_id").index
        result_series = proportion_series.reindex(base_fixture_ids).fillna(
            1.0
        )  # Default for no events is 1.0 (tied 0-0)
        return result_series
    except Exception as e:
        print(
            f"Error calculating proportion_of_game_time_score_is_tied_at_end: {str(e)}. Filling with NaNs."
        )
        result_series = pd.Series(
            np.nan,
            index=fixtures_info_df["fixture_id"].unique(),
...
def _get_fixtures_from_db() -> pd.DataFrame:
    ...
    try:
        if not all([db_host, db_name, db_user, db_password, db_port]):
            print(
                "Database environment variables not fully set. Falling back to sample fixtures data."
            )
        ...
        if not all(col in fixtures_df.columns for col in required_cols):
            print(
                f"Fetched fixtures_df is missing one of required columns: {required_cols}. Falling back to sample data."
            )
            return pd.DataFrame(sample_fallback_fixtures_data)
...
def _get_events_from_db() -> pd.DataFrame:
    ...
    try:
        if not all([db_host, db_name, db_user, db_password, db_port]):
            print(
                "Database environment variables not fully set. Falling back to sample events data."
            )
        ...
        if "time_extra" in events_df.columns:
            events_df["time_elapsed"] = events_df.apply(
                ...
                if pd.notnull(row["time_extra"])
                else row["time_elapsed"],
                axis=1,
            )
        ...
        if not all(col in events_df.columns for col in required_cols):
            print(
                f"Fetched events_df is missing one of required columns: {required_cols}. Falling back to sample data."
            )
            return pd.DataFrame(sample_fallback_events_data)
...
def main_test():
    ...
    if not fixtures_df.empty:
        event_features = calculate_event_features_vectorized(
            events_df.copy(), fixtures_df.copy()
        )  # Pass copies
