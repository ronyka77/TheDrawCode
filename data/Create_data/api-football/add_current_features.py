...
class MongoDBFeatures:
        ...
        for fixture in fixtures_with_predictions:
            try:
                ...
                fixture_data = {
                    ...
                    "home_win_percent": predictions.get("percent", {})
                    .get("home", "0%")
                    .rstrip("%"),
                    "draw_percent": predictions.get("percent", {}).get("draw", "0%").rstrip("%"),
                    "away_win_percent": predictions.get("percent", {})
                    .get("away", "0%")
                    .rstrip("%"),
                ...
                for category in [
                    ...
                    if category in comparison:
                        fixture_data[f"comparison_home_{category}"] = (
                            comparison[category].get("home", "0%").rstrip("%")
                        )
                        fixture_data[f"comparison_away_{category}"] = (
                            comparison[category].get("away", "0%").rstrip("%")
                        )
                ...
                if h2h_matches:
                    home_wins = sum(
                        1 for match in h2h_matches if match["teams"]["home"]["winner"] is True
                    )
                    away_wins = sum(
                        1 for match in h2h_matches if match["teams"]["away"]["winner"] is True
                    )
                    draws = sum(
                        1 for match in h2h_matches if match["teams"]["home"]["winner"] is None
                    )
                    fixture_data.update(
                        {
    ...
    def normalize_fixtures_data(self, fixtures_with_stats: list[dict]) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                fixtures_dataframe["date"] = pd.to_datetime(
                    fixtures_dataframe["date"], errors="coerce"
                )
                # Convert to string format only if datetime parsing succeeded
                valid_dates = fixtures_dataframe["date"].notnull()
                fixtures_dataframe.loc[valid_dates, "date"] = fixtures_dataframe.loc[
                    valid_dates, "date"
                ...
                if fixtures_dataframe[col].dtype == "object":
                    ...
                    try:
                        if fixtures_dataframe[col].str.contains("%").any():
                            fixtures_dataframe[col] = (
                                fixtures_dataframe[col].str.rstrip("%").astype(float) / 100.0
                            )
                            if "_accuracy" not in col:
                                fixtures_dataframe.rename(
                                    columns={col: col + "_accuracy"}, inplace=True
                                )
                    except Exception as e:
    ...
    def load_and_prepare_data(self, fixtures_dataframe: pd.DataFrame):
        try:
            ...
            data["home_defensive_activity"] = (
                data["home_blocked_shots"]
                + data["home_yellow_cards"]
                + data["home_red_cards"]
                + data["home_prevented_goals"]
            )
            data["away_defensive_activity"] = (
                data["away_blocked_shots"]
                + data["away_yellow_cards"]
                + data["away_red_cards"]
                + data["away_prevented_goals"]
            )
    ...
    def add_cumulative_sums(self, dataframe):
        try:
            ...
            cumsum_home_df.loc[:, "home_points_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_points"].cumsum()
            cumsum_home_df.loc[:, "home_saves_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_saves"].cumsum()
            cumsum_home_df.loc[:, "home_shots_on_target_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_shots_on_goal"].cumsum()
            cumsum_home_df.loc[:, "home_passes_accuracy_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_passes_accuracy"].cumsum()
            cumsum_home_df.loc[:, "home_fouls_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_fouls"].cumsum()
            cumsum_home_df.loc[:, "home_goal_difference_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_goal_difference"].cumsum()
            cumsum_home_df.loc[:, "home_win_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_win"].cumsum()
            cumsum_home_df.loc[:, "draw_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["draw"].cumsum()
            cumsum_home_df.loc[:, "home_count"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "fixture_id"
                ].cumcount()
                + 1
            )
            cumsum_home_df.loc[:, "red_cards_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_red_cards"].cumsum()
            cumsum_home_df.loc[:, "yellow_cards_cumsum"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["home_yellow_cards"].cumsum()

            cumsum_away_df = dataframe[cumulative_columns]
            cumsum_away_df = cumsum_away_df.copy()

            cumsum_away_df.loc[:, "away_points_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_points"].cumsum()
            cumsum_away_df.loc[:, "away_fouls_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_fouls"].cumsum()
            cumsum_away_df.loc[:, "away_saves_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_saves"].cumsum()
            cumsum_away_df.loc[:, "away_shots_on_target_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_shots_on_goal"].cumsum()
            cumsum_away_df.loc[:, "away_passes_accuracy_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_passes_accuracy"].cumsum()
            cumsum_away_df.loc[:, "away_goal_difference_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_goal_difference"].cumsum()
            cumsum_away_df.loc[:, "away_win_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_win"].cumsum()
            cumsum_away_df.loc[:, "draw_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["draw"].cumsum()
            cumsum_away_df.loc[:, "away_count"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "fixture_id"
                ].cumcount()
                + 1
            )
            cumsum_away_df.loc[:, "red_cards_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_red_cards"].cumsum()
            cumsum_away_df.loc[:, "yellow_cards_cumsum"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["away_yellow_cards"].cumsum()
            ...
            dataframe["home_cumcount"] = (
                dataframe.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "fixture_id"
                ].cumcount()
                + 1
            )
            dataframe["away_cumcount"] = (
                dataframe.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "fixture_id"
                ].cumcount()
                + 1
            )
    ...
    def add_rolling_averages(self, dataframe):
        try:
            ...
            rolling_home_df = dataframe[rolling_columns].sort_values(
                by=["home_encoded", "season_encoded", "league_encoded", "year", "week_of_year"]
            )
            rolling_away_df = dataframe[rolling_columns].sort_values(
                by=["away_encoded", "season_encoded", "league_encoded", "year", "week_of_year"]
            )
            ...
            home_df = (
                rolling_home_df.groupby(
                    ["home_encoded", "season_encoded", "league_encoded", "year", "week_of_year"]
                )
                .rolling(window=4, min_periods=1, closed="left")
                .sum()
                .reset_index()
            )
            ...
            away_df = (
                rolling_away_df.groupby(
                    ["away_encoded", "season_encoded", "league_encoded", "year", "week_of_year"]
                )
                .rolling(window=4, min_periods=1, closed="left")
                .sum()
                .reset_index()
            ...
            dataframe["home_points_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["home_encoded"],
                    home_df,
                    away_df,
                    row["league_encoded"],
            ...
            dataframe["home_passes_accuracy_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["home_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["home_win_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["home_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["home_draw_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["home_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["home_red_cards_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["home_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["away_points_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["away_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["away_passes_accuracy_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["away_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["away_win_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["away_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["away_draw_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["away_encoded"],
                    home_df,
                    away_df,
            ...
            dataframe["away_red_cards_rolling"] = dataframe.apply(
                lambda row: self.calculate_rolling_values(
                    row["away_encoded"],
                    home_df,
                    away_df,
    ...
    def calculate_rolling_values(
        self, team, home_df, away_df, league, season, year, week, home_column, away_column
    ):
        try:
            ...
            print(
                f"Error calculating rolling values(probably column not in rolling_colums list): {e}"
            )
            return 0
    ...
    def get_future_matches(self, training_data: pd.DataFrame):
        ...
        try:
            ...
            df = pd.merge(df, merge_df, on="fixture_id", how="left")
...
def main():
    ...
    mongodb_features.normalize_predictions_data(fixtures_with_predictions)
