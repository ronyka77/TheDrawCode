...
class MongoDBFeatures:
    ...
    def add_cumulative_sums(self, dataframe):
        try:
            ...
            cumsum_home_df.loc[:, "home_points_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_points"
                ].cumsum()
                - cumsum_home_df["home_points"]
            )
            cumsum_home_df.loc[:, "home_saves_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_saves"
                ].cumsum()
                - cumsum_home_df["home_saves"]
            )
            cumsum_home_df.loc[:, "home_shots_on_target_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_shots_on_goal"
                ].cumsum()
                - cumsum_home_df["home_shots_on_goal"]
            )
            cumsum_home_df.loc[:, "home_passes_accuracy_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_passes_accuracy"
                ].cumsum()
                - cumsum_home_df["home_passes_accuracy"]
            )
            cumsum_home_df.loc[:, "home_fouls_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_fouls"
                ].cumsum()
                - cumsum_home_df["home_fouls"]
            )
            cumsum_home_df.loc[:, "home_goal_difference_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_goal_difference"
                ].cumsum()
                - cumsum_home_df["home_goal_difference"]
            )
            cumsum_home_df.loc[:, "home_win_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_win"
                ].cumsum()
                - cumsum_home_df["home_win"]
            )
            cumsum_home_df.loc[:, "draw_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "draw"
                ].cumsum()
                - cumsum_home_df["draw"]
            )
            cumsum_home_df.loc[:, "home_count"] = cumsum_home_df.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["fixture_id"].cumcount()
            cumsum_home_df.loc[:, "red_cards_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_red_cards"
                ].cumsum()
                - cumsum_home_df["home_red_cards"]
            )
            cumsum_home_df.loc[:, "yellow_cards_cumsum"] = (
                cumsum_home_df.groupby(["home_encoded", "season_encoded", "league_encoded"])[
                    "home_yellow_cards"
                ].cumsum()
                - cumsum_home_df["home_yellow_cards"]
            )

            cumsum_away_df = dataframe[cumulative_columns]
            cumsum_away_df = cumsum_away_df.copy()

            cumsum_away_df.loc[:, "away_points_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_points"
                ].cumsum()
                - cumsum_away_df["away_points"]
            )
            cumsum_away_df.loc[:, "away_fouls_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_fouls"
                ].cumsum()
                - cumsum_away_df["away_fouls"]
            )
            cumsum_away_df.loc[:, "away_saves_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_saves"
                ].cumsum()
                - cumsum_away_df["away_saves"]
            )
            cumsum_away_df.loc[:, "away_shots_on_target_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_shots_on_goal"
                ].cumsum()
                - cumsum_away_df["away_shots_on_goal"]
            )
            cumsum_away_df.loc[:, "away_passes_accuracy_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_passes_accuracy"
                ].cumsum()
                - cumsum_away_df["away_passes_accuracy"]
            )
            cumsum_away_df.loc[:, "away_goal_difference_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_goal_difference"
                ].cumsum()
                - cumsum_away_df["away_goal_difference"]
            )
            cumsum_away_df.loc[:, "away_win_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_win"
                ].cumsum()
                - cumsum_away_df["away_win"]
            )
            cumsum_away_df.loc[:, "draw_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "draw"
                ].cumsum()
                - cumsum_away_df["draw"]
            )
            cumsum_away_df.loc[:, "away_count"] = cumsum_away_df.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["fixture_id"].cumcount()
            cumsum_away_df.loc[:, "red_cards_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_red_cards"
                ].cumsum()
                - cumsum_away_df["away_red_cards"]
            )
            cumsum_away_df.loc[:, "yellow_cards_cumsum"] = (
                cumsum_away_df.groupby(["away_encoded", "season_encoded", "league_encoded"])[
                    "away_yellow_cards"
                ].cumsum()
                - cumsum_away_df["away_yellow_cards"]
            )
            ...
            path_cumsum_away = (
                "data/Create_data/data_files/base/cumsum_away_last_match_features.xlsx"
            )
            path_cumsum_home = (
                "data/Create_data/data_files/base/cumsum_home_last_match_features.xlsx"
            )
            ...
            dataframe["home_cumcount"] = dataframe.groupby(
                ["home_encoded", "season_encoded", "league_encoded"]
            )["fixture_id"].cumcount()
            dataframe["away_cumcount"] = dataframe.groupby(
                ["away_encoded", "season_encoded", "league_encoded"]
            )["fixture_id"].cumcount()
