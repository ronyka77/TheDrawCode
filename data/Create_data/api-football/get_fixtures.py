...
class ApiFootball:
    ...
    def get_league_ids(self) -> None:
        ...
        try:
            with open(leagues_file_path) as f:
            ...
            for item in leagues_data:
                ...
                league_info.append(
                    {"league_id": league_id, "league_name": league_name, "country": country_name}
                )
        ...
        if response and "response" in response:
            ...
            for fixture in fixtures:
                transformed_fixture = {
                    ...
                    and fixture["goals"]["home"] > fixture["goals"]["away"]
                    else (
                        2
                        if fixture["fixture"]["status"]["short"] == "FT"
                        and fixture["goals"]["home"] == fixture["goals"]["away"]
                        else (3 if fixture["fixture"]["status"]["short"] == "FT" else None)
                    ),
                }
        ...
        if response and "response" in response and response["response"]:
            ...
            try:
                self.fixtures_collection.update_one(
                    ...
                    {
                        "$set": {
                            "home.stats": statistics["home"]["stats"],
                            "away.stats": statistics["away"]["stats"],
                        }
                    },
                )
                ...
                self.logger.error(
                    f"Error inserting statistics for fixture {fixture_id} into MongoDB: {e}"
                )
            return statistics
        else:
            self.logger.warning(f"No statistics found for fixture {fixture_id}")
            try:
                fixture_date_str = self.fixtures_collection.find_one(
                    {"fixture_id": fixture_id}, {"date": 1}
                )["date"]
                ...
                if fixture_date < datetime.now() - timedelta(days=7):
                    ...
                    self.logger.info(
                        f"Fixture {fixture_id} dropped from MongoDB due to date constraint."
                    )
                    return {}
            except Exception as e:
    ...
    def get_fixture_ids_without_statistics(self) -> list[int]:
        ...
        league_ids_file_path = os.path.join(
            project_root, "data", "create_data", "api-football", "league_ids.json"
        )
        ...
        query = {
            ...
            "$or": [{"home.stats": {"$exists": False}}, {"home.stats": {}}],
            "score.fulltime.home": {"$ne": None},
        }
        fixtures = self.fixtures_collection.find(
            query, {"fixture_id": 1, "league_id": 1, "league_name": 1}
        )
    ...
    def get_fixture_ids_without_predictions(self) -> list[int]:
        ...
        existing_predictions = self.predictions_collection.distinct(
    ...
    def get_league_statistics_from_mongodb(self) -> dict[int, int]:
        ...
        try:
            ...
            pipeline = [
                {"$match": {"score.fulltime.home": {"$exists": True, "$ne": None}}},
    ...
    def delete_fixtures_not_in_leagues(self) -> None:
        ...
        try:
            ...
            if len(league_ids) > 0:
                ...
                result = self.fixtures_collection.delete_many({"league_id": {"$nin": league_ids}})
                result = self.fixtures_collection.delete_many(
                    {
                        "$or": [
                            {"league_id": {"$nin": league_ids}},
                            {"score.fulltime.home": None, "date": {"$lt": "2024-10-01"}},
                        ]
                    }
                )
    ...
    def get_teams_missing_venues(self) -> list[int]:
        ...
        try:
            ...
            team_ids = self.fixtures_collection.distinct(
                "teams.home.id"
    ...
    def get_teams_for_leagues(self) -> None:
        ...
        output_file_path = os.path.join(
            project_root, "data", "create_data", "api-football", "teams.json"
        )
            ...
            for league_info in league_ids_data:
                ...
                if response.status_code == 200:
                    ...
                    if teams_data["results"] > 0:
                        ...
                        self.logger.info(
                            f"Retrieved {len(teams_data['response'])} teams for league ID: {league_id}"
                        )
                        ...
                        for team in teams_data["response"]:
                            try:
                                ...
                                self.logger.error(
                                    f"Error updating venue data for team ID {team['team']['id']}: {e}"
                                )
                    ...
                    self.logger.error(
                        f"Error getting teams for league ID {league_id}: {response.status_code}"
                    )
            ...
            if response.status_code == 200:
                ...
                if predictions_data.get("response"):
                    ...
                    prediction_doc = {
                        ...
                        "teams": {
                            "home": {
                                ...
                                "last_5": prediction.get("teams", {})
                                .get("home", {})
                                .get("last_5", {}),
                                "league": prediction.get("teams", {})
                                .get("home", {})
                                .get("league", {}),
                            },
                            "away": {
                    ...
                    self.predictions_collection.update_one(
                        {"fixture_id": fixture_id}, {"$set": prediction_doc}, upsert=True
                    )
                ...
                self.logger.error(
                    f"Error getting prediction data for fixture ID {fixture_id}: {response.status_code}"
                )
                ...
                for team_id in missing_team_ids:
                    ...
                    if response.status_code == 200:
                        ...
                        if team_data["results"] > 0:
                            ...
                            self.venues_collection.update_one(
                                {"team.id": team_id}, {"$set": venue_doc}, upsert=True
                            )
