...
class FeatureEngineer_api_football:
    def __init__(self,
        ...
        self.processing_config = {
            ...
            'data_paths': {
                'raw_data_path': base_config.get('data_paths', {}).get('raw_data_path', ''),
                'processed_data_path': base_config.get('data_paths', {}).get('processed_data_path', ''),
                'cache_path': base_config.get('data_paths', {}).get('cache_path', '')
            }
        }
        
        self.pipeline_config = load_config('pipeline')
        self.data_loader = DataLoader(
            data_path=self.processing_config['data_paths']['raw_data_path'],
    ...
    def engineer_draw_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if all(col in df.columns for col in ['league_encoded', 'home_encoded', 'match_outcome']):
                league_home_draw_rates = df.groupby(['league_encoded', 'home_encoded'], group_keys=False)['match_outcome'].apply(
                    lambda x: (x == 2).mean()
                ).reset_index()
            ...
            if all(col in df.columns for col in ['league_encoded', 'away_encoded', 'match_outcome']):
                league_away_draw_rates = (
                    df.groupby(['league_encoded', 'away_encoded'], group_keys=False)['match_outcome']
                    .mean()
                    .reset_index()
    ...
    def add_league_specific_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'team_strength_diff' in df.columns:
                league_competitiveness = df.groupby('league_encoded', observed=True).apply(
    ...
    def add_enhanced_draw_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                # Group by league to calculate form convergence within each league
                df['form_convergence'] = df.groupby('league_encoded', group_keys=False, observed=True).apply(
                    # For each group (league), calculate the form convergence
                    lambda x: 1 - abs(
                        # Calculate the difference in form momentum between home and away teams
                        x['home_form_momentum'].diff().rolling(5).mean() - 
                ...
                self.logger.info('form_convergence')
                df['form_convergence'] = pd.to_numeric(df['form_convergence'], errors='coerce').fillna(0)
            except Exception as e:
                self.logger.error(f"Error in form_convergence: {str(e)}")
                
            # League position volatility
            try:    
                self.logger.info('position_volatility')
                # Calculate rolling standard deviation with proper index alignment
                position_volatility = df.groupby('league_encoded', group_keys=False, observed=True)['team_strength_diff']\
                    .rolling(5, min_periods=1).std()  # Use min_periods=1 to avoid NaNs for small windows

                # Ensure the result aligns with the original DataFrame's index
                df['position_volatility'] = position_volatility.reset_index(level=0, drop=True).fillna(0)
            except Exception as e:
                self.logger.error(f"Error in position_volatility: {str(e)}")
    ...
    def add_advanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            all_matches = df[['fixture_id','home_encoded','away_encoded','date_encoded','is_draw']]
            ...
            if all(col in df.columns for col in ['home_points_cum', 'Away_points_cum']):
                ...
                df['home_points_acceleration'] = df.groupby('home_encoded', group_keys=False)['home_points_cum'].rolling(window=5).apply(lambda x: np.polyfit(range(len(x)), x, 2)[0]).reset_index(level=0, drop=True)
                df['away_points_acceleration'] = df.groupby('away_encoded', group_keys=False)['Away_points_cum'].rolling(window=5).apply(lambda x: np.polyfit(range(len(x)), x, 2)[0]).reset_index(level=0, drop=True)
    ...
    def load_and_process_data(
        ...
        try:
            ...
            self.data_loader = DataLoader(
                ...
                logger=self.logger,
                is_prediction=False  #IMPORTANT: This is for training data
            )
            ...
            preprocessed_data = self.engineer_all_features(preprocessed_data)
            ...
            self.logger.error(f"Error in load_and_process_data: {str(e)}")
            return None
...
if __name__ == "__main__":
    ...
    file_paths = {
        "prediction_base": 'data/prediction_raw/api_football_prediction_newPoisson.xlsx',
        "training_base": 'data/prediction_raw/api_football_training_newPoisson.xlsx',
        "training_new": 'data/prediction/api_preprocessed_data_training.xlsx',
        "prediction_new": 'data/prediction/api_preprocessed_data_prediction.xlsx'
    }
    # Initialize the feature engineer
    logger = ExperimentLogger()
    feature_engineer = FeatureEngineer_api_football(logger=logger, target_variable='match_outcome')
    
    df = feature_engineer.load_and_process_data(file_paths["prediction_base"], file_paths["prediction_new"])

    df2 = feature_engineer.load_and_process_data(file_paths["training_base"], file_paths["training_new"])
