...
logger = LoggerSetup.setup_logger(
    name="aggregation", log_file="./log/aggregation.log", level=logging.INFO
)
...
def aggregate_data():
    ...
    try:
        ...
        aggregated_df = pd.merge(
            matches_df, stats_df, how="left", on="unique_id", suffixes=("_match", "_stats")
        )
    ...
    try:
        ...
        for record in records:
            ...
            bulk_operations.append(
                UpdateOne({"unique_id": record["unique_id"]}, {"$set": record}, upsert=True)
            )
