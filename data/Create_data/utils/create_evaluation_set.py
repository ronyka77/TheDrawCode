...
logger = ExperimentLogger(
    log_dir="logs/create_evaluation_set", experiment_name="create_evaluation_set"
)
...
def create_parquet_files(
    ...
    try:
        if partition_cols:
            data.to_parquet(
                output_path, partition_cols=partition_cols, engine="pyarrow", index=False
            )
    ...
    try:
        ...
        if model_type is not None:
            if model_type not in ["xgb", "cat", "lgbm"]:
                raise ValueError(
                    f"Invalid model_type: {model_type}. Must be one of: 'xgb', 'cat', 'lgbm'"
                )
...
def update_api_data_for_draws():
    ...
    try:
        ...
        api_prediction_eval = updated_data[
            (updated_data["Date"] >= "2024-11-01") & (updated_data["match_outcome"].notna())
        ]

        # Filter data for dates after 2024-11-01 where match_outcome is blank
        api_prediction_data = updated_data[
            (updated_data["Date"] >= "2025-01-15") & (updated_data["match_outcome"].isna())
        ]
        ...
        print("api_training_final.xlsx updated")

        # Export df_after_2024_11_01_not_blank to data/prediction/api_predictions_eval.xlsx
        api_prediction_eval.to_excel(
            "data/prediction/api_prediction_eval.xlsx", index=False, engine="xlsxwriter"
        )
        print("api_prediction_eval.xlsx updated")

        # Export df_after_2024_11_01_blank to data/prediction/api_predictions_data.xlsx
        api_prediction_data.to_excel(
            "data/prediction/api_predictions_data.xlsx", index=False, engine="xlsxwriter"
        )
        print("api_predictions_data.xlsx updated")
...
def import_training_data_draws_api() -> tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
    ...
    try:
        ...
        if os.path.exists(parquet_path):
            ...
            if data.empty:
                logger.error(
                    "Loaded dataset is empty", error_code=DataProcessingError.EMPTY_DATASET
                )
...
def create_ensemble_evaluation_set() -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(
            f"Data file not found: {str(e)}", error_code=DataProcessingError.FILE_NOT_FOUND
        )
        raise
    except ValueError as e:
        logger.error(
    ...
    if os.path.exists(parquet_path):
        try:
            ...
            if "is_draw" not in data.columns:
                logger.warning("is_draw column not found in parquet file, creating target variable")
                data["is_draw"] = (data["match_outcome"] == 2).astype(int)
        except Exception as e:
            logger.warning(f"Failed to load parquet file, falling back to Excel: {str(e)}")
            data = pd.read_excel(data_path)
    else:
        data = pd.read_excel(data_path)
...
def get_selected_columns_from_mlflow_run(run_id: str) -> list[str]:
    ...
    try:
        ...
        raise ValueError(f"Invalid MLflow run ID {run_id}: {str(e)}") from e
    except FileNotFoundError as e:
        raise FileNotFoundError(f"MLmodel artifact not found in run {run_id}") from e
    except Exception as e:
        raise Exception(f"Error retrieving features from MLflow run {run_id}: {str(e)}") from e
