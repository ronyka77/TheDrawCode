...
class EnhancedFeatureSelector(BaseEstimator):
    ...
    def _calculate_composite_scores(
        self, X: pd.DataFrame, y: pd.Series, model: xgb.XGBClassifier, weights: dict[str, float]
    ) -> dict[str, float]:
    ...
    def _select_top_features(
        self, scores: dict[str, float], X: pd.DataFrame, min_features: int = 50
    ) -> list[str]:
    ...
    def _evaluate_feature_set(
        self, X: pd.DataFrame, y: pd.Series, model: xgb.XGBClassifier, cv: int = 5
    ) -> float:
        ...
        try:
            ...
            cv_model = xgb.XGBClassifier(
                **{
                ...
                }  # Remove eval_metric for CV
            )

            # Perform cross-validation
            scores = cross_val_score(cv_model, X, y, cv=cv, scoring="average_precision")
            return scores.mean()
        except Exception as e:
    ...
    def analyze_correlations(
        self, X: pd.DataFrame, selected_features: list[str]
    ) -> list[list[str]]:
    ...
    def perform_stability_selection(
        ...
        stable_features = {
            f: s
            for f, s in stability_scores.items()
    ...
    def perform_iterative_elimination(
        ...
        while len(remaining_features) > min_features:
            try:
                ...
                mlflow.log_metrics({"current_score": score, "n_features": len(remaining_features)})
        ...
        mlflow.log_metrics({"best_score": best_score, "n_selected_features": len(best_features)})

        return best_features

    def select_features(self, X: pd.DataFrame, y: pd.Series, model: xgb.XGBClassifier) -> list[str]:
        ...
        try:
            ...
            stability_scores = self.perform_stability_selection(X, y, model, best_weights)
            ...
            for group in correlation_groups:
                ...
                best_feature = max(group, key=lambda x: stability_scores.get(x, 0))
                unique_features.append(best_feature)

            # Add uncorrelated features
            uncorrelated = [
                f for f in stable_features if not any(f in group for group in correlation_groups)
            ]
...
class PrecisionFocusedFeatureSelector(BaseEstimator):
    ...
    def _calculate_interaction_importance(
        self, model: xgb.XGBClassifier, X: pd.DataFrame, y: pd.Series, feature_names: list[str]
    ) -> np.ndarray:
    ...
    def _calculate_precision_impact(
        ...
        for i, feature in enumerate(feature_names):
            try:
                ...
                self.logger.error(
                    f"Error calculating precision impact for feature {feature}: {str(e)}"
                )
    ...
    def select_features(
        ...
        try:
            ...
            self.logger.info(
                f"Starting feature selection with {len(valid_features)} valid features"
            )
            ...
            for feature in valid_features:
                try:
                    ...
                    self._group_feature(
                        feature, correlations, selected, feature_groups, correlation_threshold
                    )
    ...
    def _calculate_safe_correlations(
        self, X: pd.DataFrame, selected: list[str], feature: str
    ) -> pd.Series:
    ...
    def _select_best_features(
        self, feature_groups: list[list[str]], importance_df: pd.DataFrame
    ) -> list[str]:
        ...
        for group in feature_groups:
            try:
                best_feature = max(
                    group,
                    key=lambda x: importance_df.loc[
...
def run_feature_selection(experiment_name: str = "feature_selection_optimization") -> list[str]:  # type: ignore
    ...
    Returns:
        List of selected feature names
    """
    logger = ExperimentLogger(experiment_name=experiment_name, log_dir="logs/feature_selection")
    ...
    try:
        ...
        precision_selector = PrecisionFocusedFeatureSelector(
            min_recall=0.20, target_precision=0.50, logger=logger
        )

        # Initialize base model for feature selection
        base_model = xgb.XGBClassifier(
            tree_method="hist", device="cpu", random_state=42, eval_metric=["auc", "aucpr"]
        )

        with mlflow.start_run(
            run_name=f"precision_focused_feature_selection_{datetime.now().strftime('%Y%m%d_%H%M')}"
        ):
            ...
            mlflow.log_metrics(
                {
                    ...
                    "feature_reduction_percent": (1 - len(final_features) / len(standard_features))
                    * 100,
                }
            )
            ...
            logger.info("\nMetrics Comparison:")
            logger.info(
                f"Standard - Precision: {standard_metrics['precision']:.4f}, Recall: {standard_metrics['recall']:.4f}"
            )
            logger.info(
                f"Precision-Focused - Precision: {precision_metrics['precision']:.4f}, Recall: {precision_metrics['recall']:.4f}"
            )
...
def align_columns(
    ...
    common_columns = (
        set(train_df.columns).intersection(set(eval_df.columns)).intersection(set(test_df.columns))
    )
