...
class DuplicateHandler:
    ...
    def __init__(self, collection_name: str = "fixtures"):
        ...
        self.logger = LoggerSetup.setup_logger(
            name="delete_duplicates", log_file="./log/delete_duplicates.log", level=logging.INFO
        )
        try:
            ...
            with self.client.get_database() as db:
                ...
                self.logger.info(
                    f"Successfully connected to database. Found {doc_count} documents."
                )
    ...
    def find_duplicates(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            with self.client.get_database() as db:
                ...
                self.logger.info(
                    f"Processing {len(documents)} documents. This may take a few minutes..."
                )
                ...
                for doc in documents:
                    ...
                    collection.update_one(
                        {"_id": doc["_id"]}, {"$set": {"Home": std_home, "Away": std_away}}
                    )
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Invalid date format for document {doc.get('unique_id')}: {doc.get('Date')}"
                        )
                        doc["date_obj"] = None
                ...
                for i in range(0, len(documents), batch_size):
                    ...
                    self.logger.info(
                        f"Processing batch {i // batch_size + 1} of {len(documents) // batch_size + 1}"
                    )
                    print(
                        f"Processing batch {i // batch_size + 1} of {len(documents) // batch_size + 1}"
                    )
                    for doc in batch:
                        ...
                        match_found = False
                        for _key, existing in seen_matches.items():
                            existing_date = existing["date"]
                            date_diff = abs((doc["date_obj"] - existing_date).days)

                            if (
                                date_diff <= 2
                                and doc["Home"] == existing["home"]
                                and doc["Away"] == existing["away"]
                            ):
                                # This is a duplicate
                                ...
                                odds_update = {
                                    k: doc[k]
                                    for k in ["Odd_Home", "Odd_Away", "Odds_Draw"]
                                    if k in doc and doc[k]
                                }

                                if odds_update:
                                    collection.update_one(
                                        {"_id": original_id}, {"$set": odds_update}
                                    )
                                break
    ...
    def delete_duplicates(self) -> None:
        ...
        try:
            ...
            with self.client.get_database() as db:
                ...
                self.logger.info(
                    f"Successfully deleted {result.deleted_count} duplicate documents from {self.collection_name}"
                )
