...
# Initialize logger
logger = ExperimentLogger(experiment_name="data_ingestion", log_dir="logs/data_ingestion")
    ...
    def validate_dataset(
        ...
        try:
            ...
            if is_training and len(data) < self.MIN_SAMPLES:
                return (
                    False,
                    f"Dataset has {len(data)} samples, minimum required is {self.MIN_SAMPLES}",
                )
...
class DataIngestion:
    ...
    def load_evaluation_data(self) -> tuple[pd.DataFrame, pd.Series]:
        ...
        try:
            ...
            logger.info(
                ...
                extra={"eval_shape": X_eval.shape, "eval_draws": int(y_eval.sum())},
            )
    ...
    def convert_to_mlflow_format(self, data: Union[pd.DataFrame, np.ndarray]) -> pd.DataFrame:
...
def get_feature_metadata() -> dict[str, dict[str, str]]:
    ...
    try:
        ...
        for model_type in ["xgb", "cat", "all"]:
            ...
            metadata[model_type] = {
                feature: {"type": "numeric", "description": "Feature selected for prediction"}
