...
async def start_training_job(background_tasks: BackgroundTasks, config: TrainingConfig = None):
    ...
    return TrainingResponse(
        job_id=job_id, status="submitted", message=f"Training job {job_id} submitted successfully."
    )
...
async def get_training_results(job_id: str):
    ...
    if job_id not in job_results:
        raise HTTPException(
            status_code=404,
            detail=f"Results not found for job_id: {job_id}. Job may not exist, be running, or failed.",
        )
...
async def make_prediction_from_file(file: UploadFile = File(...)):  # noqa: B008
    """
    ...
    try:
        ...
        if df.empty:
            raise HTTPException(
                status_code=400, detail="The uploaded XLSX file is empty or has no data."
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to load prediction model: {str(model_load_error)}"
        ...
        print(
            f"Prediction completed. Processing {prediction_results_dict.get('num_predictions', 0)} predictions."
        )
        ...
        raise HTTPException(status_code=400, detail="The uploaded XLSX file is empty.") from e
    except ValueError as ve:
        # Catch validation errors from DrawPredictor (_validate_input)
        print(f"Input validation error: {ve}")
        raise HTTPException(status_code=400, detail=f"Input validation error: {str(ve)}") from ve
    except Exception as e:
        print(f"Error processing prediction file {file.filename}: {e}")
        # Raise a generic server error, consider more specific errors
        raise HTTPException(
            status_code=500, detail=f"Failed to process prediction file: {str(e)}"
        ) from e
