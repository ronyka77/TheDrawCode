...
class TrainingConfig(BaseModel):
    ...
    meta_learner_type: str = Field(
        default="lgb", description="Type of the meta-learner ('lgb', 'xgb', 'logistic', 'mlp')"
    )
    calibrate: bool = Field(
        default=False, description="Whether to calibrate base model probabilities"
    )
    dynamic_weighting: bool = Field(
        default=True, description="Whether to use dynamic weighting for base models"
    )
    target_precision: float = Field(
        default=0.50, ge=0, le=1, description="Target precision for threshold optimization"
    )
    required_recall: float = Field(
        default=0.25, ge=0, le=1, description="Minimum required recall for threshold optimization"
    )
